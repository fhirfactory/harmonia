...
/**
 * Main command and CLI interface for the Mnemosyne Operations JPA Server.
 */
@Command(
        name = "mnemosyne-operations-cli",
        mixinStandardHelpOptions = true,
        version = "mnemosyne-operations-cli 1.0.0",
        description = "Command-line tool for interacting with the Mnemosyne Operations JPA Server and managing queues, task sequences, and operational resources.",
        subcommands = {
...
public class OperationsCliCommand implements Callable<Integer> {
    ...
    public String resolveEffectiveServerUrl() {
        ...
        String envOpsUrl = System.getenv("OPS_SERVER_URL");
        if (envOpsUrl != null && !envOpsUrl.isBlank()) {
            return envOpsUrl;
        }
        String envHarmoniaUrl = System.getenv("HARMONIA_OPERATIONS_SERVER_URL");
        if (envHarmoniaUrl != null && !envHarmoniaUrl.isBlank()) {
            return envHarmoniaUrl;
        }
