# Harmonia Mnemosyne Operations CLI (`mnemosyne-operations-cli`)
...
```bash
mvn clean package -pl hestia/mnemosyne-operations-cli -am
```

The build produces a self-contained shaded executable JAR at:
```
hestia/mnemosyne-operations-cli/target/mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar
```
...
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar --list-sequences

# Pretty JSON View
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar list-sequences -o json --pretty

# Targeting Remote Operations Server
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar -s http://operations-node-1:8080/api/operations --list-sequences
```
...
```bash
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar get-sequence seq-admission-pipeline
```
...
From a JSON file:
```bash
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar save-sequence -f custom-sequence.json
```

From inline JSON:
```bash
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar save-sequence -d '{"sequenceId":"seq-custom","sequenceName":"Custom Pipeline","enabled":true,"targetGatewayInstances":["*"],"targetTriggerTypes":["A01"],"activityIds":["patient-identity-update"]}'
```

### 4. Deleting a TaskSequence

```bash
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar delete-sequence seq-custom
```
...
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar list-resources

# List all resources for a specific object type (e.g. config)
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar list-resources -t config

# Get a specific resource
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar get-resource config cluster-settings

# Save a resource
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar save-resource config cluster-settings -d '{"maxNodes":5,"mode":"clustered"}'

# Check if a resource exists
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar check-resource config cluster-settings

# Delete a resource
java -jar mnemosyne-operations-cli-1.0.0-SNAPSHOT.jar delete-resource config cluster-settings
```
