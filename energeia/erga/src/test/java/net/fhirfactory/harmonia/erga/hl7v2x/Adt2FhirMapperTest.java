...
class Adt2FhirMapperTest {
    ...
    void testMapperMetadataAndInheritance() {
        assertThat(mapper).isInstanceOf(ErgonBase.class);
    ...
    void testCamelExchangeExecution() throws Exception {
        ...
        ErgonEvent ergonEvent = new ErgonEvent("TASK-EXCHANGE-MAP", "PROCESS", "requested");
        exchange.getMessage().setBody(ergonEvent);
        exchange.getMessage().setHeader(ErgonBase.HEADER_TASK_ID, "TASK-EXCHANGE-MAP");
        ...
        @SuppressWarnings("unchecked")
        List<Provenance> provenances = (List<Provenance>) resultExchange.getProperty(ErgonBase.PROPERTY_PROVENANCES);
        assertThat(provenances).hasSize(2);

        // Check TaskEvents generated in egress
        @SuppressWarnings("unchecked")
        List<ErgonEvent> ergonEvents = (List<ErgonEvent>) resultExchange.getProperty(ErgonBase.PROPERTY_TASK_EVENTS);
        assertThat(ergonEvents).hasSize(2);
    }
    ...
    @Test
    void testExtractCommunicationAndAdtMessageFromTask() {
        Task task = new Task();
        task.setId("Task/TASK-COMM-EXTRACT");
        task.setStatus(Task.TaskStatus.INPROGRESS);

        Communication comm = new Communication();
        comm.setId("comm-test-99");
        Attachment att = new Attachment();
        att.setContentType("application/hl7-v2");
        att.setData(FULL_ADT_A01.getBytes(StandardCharsets.UTF_8));
        ...
        input.getType().setText("HL7 Communication Resource").addCoding()
                .setCode("input-communication");
        input.setValue(new Reference("#comm-test-99"));

        Communication extractedComm = mapper.extractCommunicationFromTask(task);
        assertThat(extractedComm).isNotNull();
        assertThat(extractedComm.getIdPart()).isEqualTo("comm-test-99");

        AdtMessageExtractor.ExtractedInputResult extractedResult = mapper.extractAdtMessageFromTask(task, null);
        assertThat(extractedResult).isNotNull();
        assertThat(extractedResult.getRawMessage()).isEqualTo(FULL_ADT_A01);
        assertThat(extractedResult.getOriginInput()).isEqualTo(input);
    }
}
