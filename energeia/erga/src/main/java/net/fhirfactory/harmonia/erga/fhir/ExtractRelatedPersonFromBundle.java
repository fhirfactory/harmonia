 ...
 */
@Dependent
public class ExtractRelatedPersonFromBundle extends ErgonBase {
    ...
    protected void processErgon(Pragma pragma, Exchange exchange) throws Exception {
        ...
        if (!extractedRelatedPersons.isEmpty()) {
            RelatedPerson firstRel = extractedRelatedPersons.get(0);
            String relId = firstRel.getIdPart();
            String fullName = extractRelatedPersonFullName(firstRel);
            String patientRef = firstRel.hasPatient() ? firstRel.getPatient().getReference() : null;
            String relationship = extractRelatedPersonRelationship(firstRel);

            if (StringUtils.isNotBlank(relId)) {
                exchange.getMessage().setHeader(HEADER_RELATED_PERSON_ID, relId);
            }
