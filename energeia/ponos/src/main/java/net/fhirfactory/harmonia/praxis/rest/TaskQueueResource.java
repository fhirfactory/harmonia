...
public class TaskQueueResource {
    ...
    @Inject
    private net.fhirfactory.harmonia.praxis.sequence.TaskSequenceLoader taskSequenceLoader;
    ...
    public Response enqueueTaskEvent(String payload) {
        ...
        try {
            ErgonEvent ergonEvent = objectMapper.readValue(payload, ErgonEvent.class);
            if (StringUtils.isBlank(ergonEvent.getTaskId())) {
                return Response.status(Response.Status.BAD_REQUEST)
            ...
            producerService.sendTaskEvent(ergonEvent);

            Map<String, Object> result = new HashMap<>();
            result.put("status", "ENQUEUED");
            result.put("queueName", queueConfig.getEventQueueName());
            result.put("taskId", ergonEvent.getTaskId());
            result.put("action", ergonEvent.getAction());
            return Response.accepted(result).build();
        } catch (Exception e) {
