...
class TaskSequenceLoaderTest {
    ...
    public static class Step1Activity extends ErgonBase {
    ...
    public static class Step2Activity extends ErgonBase {
        public static final AtomicInteger executionCount = new AtomicInteger(0);
    ...
    void testSeedAndRegisterOnEmptyCache() throws Exception {
        ...
        Instance<ErgonBase> mockActivities = new MockActivityInstance<>(List.of(act1, act2, patientIdAct));

        loader = new TaskSequenceLoader(sequenceService, mockActivities);
        List<Praxis> loaded = loader.loadAndRegisterSequences(camelContext);
        camelContext.start();
    ...
    void testPatientIdentityUpdateSequencePipelineExecution() throws Exception {
        PatientIdentityUpdateErgon patientIdAct = new PatientIdentityUpdateErgon();
        Instance<ErgonBase> mockActivities = new MockActivityInstance<>(List.of(patientIdAct));

        Praxis patientSeq = new Praxis("seq-patient-identity-pipeline", "Patient Identity Update Sequence");
        ...
        patientSeq.setActivityIds(List.of(PatientIdentityUpdateErgon.DEFAULT_ACTIVITY_ID));
        ...
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_ID)).isEqualTo("MRN-SEQ-777");
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_MRN)).isEqualTo("MRN-SEQ-777");
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_NAME)).isEqualTo("MR ROBERT L WILLIAMS JR");
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_UPDATED)).isEqualTo(true);

        ErgonEvent ergonEvent = exchange.getMessage().getBody(ErgonEvent.class);
        assertThat(ergonEvent).isNotNull();
        assertThat(ergonEvent.getAction()).isEqualTo("PROCESS");
        assertThat(ergonEvent.getDescription()).contains("WILLIAMS");

        org.hl7.fhir.r5.model.Task cachedTask = patientIdAct.getTaskCacheService().getTask(ergonEvent.getTaskId()).orElse(null);
    ...
    void testPatientIdentityAndDemographicsSequencePipelineExecution() throws Exception {
        ...
        patientSeq.setTargetGatewayInstances(List.of("*"));
        patientSeq.setTargetTriggerTypes(List.of("*"));
        patientSeq.setActivityIds(List.of(PatientIdentityUpdateErgon.DEFAULT_ACTIVITY_ID, PatientDemographicsUpdateErgon.DEFAULT_ACTIVITY_ID));
        ...
        org.apache.camel.Exchange exchange = mockOut.getExchanges().get(0);
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_ID)).isEqualTo("MRN-DEMO-888");
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_MRN)).isEqualTo("MRN-DEMO-888");
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_NAME)).isEqualTo("SARAH E DAVIS MS");
        assertThat(exchange.getMessage().getHeader(PatientDemographicsUpdateErgon.HEADER_PATIENT_GENDER)).isEqualTo("female");
        assertThat(exchange.getMessage().getHeader(PatientDemographicsUpdateErgon.HEADER_PATIENT_DOB)).isEqualTo("1988-12-01");
    ...
    void testEndToEndPipelineExecutionWithMatchingEvent() throws Exception {
        ...
        Step1Activity act1 = new Step1Activity();
        Step2Activity act2 = new Step2Activity();
        Instance<ErgonBase> mockActivities = new MockActivityInstance<>(List.of(act1, act2));

        // Create and persist specific sequence in cache
        Praxis pasAdmitSequence = new Praxis("seq-pas-admissions", "PAS Admissions Sequence");
        ...
        ErgonEvent matchingEvent = new ErgonEvent("task-matching-1", "PROCESS", "REQUESTED",
    ...
    void testFilteringNonMatchingEvent() throws Exception {
        ...
        Praxis limsSequence = new Praxis("seq-lims-results", "LIMS Results Sequence");
        ...
        ErgonEvent nonMatchingEvent = new ErgonEvent("task-nonmatch-1", "PROCESS", "REQUESTED",
    ...
    void testMessageQueueConduitInTaskSequencePipeline() throws Exception {
        ...
        PatientIdentityUpdateErgon identityAct = new PatientIdentityUpdateErgon();
        PatientDemographicsUpdateErgon demographicsAct = new PatientDemographicsUpdateErgon();

        Instance<ErgonBase> mockActivities = new MockActivityInstance<>(List.of(conduit, identityAct, demographicsAct));

        Praxis seq = new Praxis("seq-conduit-test", "Conduit Pipeline Sequence");
        ...
        seq.setActivityIds(List.of(
                net.fhirfactory.harmonia.erga.infrastructure.MessageQueueToExchangeConduit.DEFAULT_ACTIVITY_ID,
        ...
        org.apache.camel.Exchange exchange = mockOut.getExchanges().get(0);
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_ID)).isEqualTo("MRN-CONDUIT-999");
        assertThat(exchange.getMessage().getHeader(PatientIdentityUpdateErgon.HEADER_PATIENT_NAME)).isEqualTo("MS EMMA J TAYLOR");
        assertThat(exchange.getMessage().getHeader(PatientDemographicsUpdateErgon.HEADER_PATIENT_GENDER)).isEqualTo("female");
        assertThat(exchange.getMessage().getHeader(PatientDemographicsUpdateErgon.HEADER_PATIENT_DOB)).isEqualTo("1992-07-04");
    }
}
