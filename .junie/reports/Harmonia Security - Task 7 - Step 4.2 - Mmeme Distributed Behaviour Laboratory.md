...
a.
Distributed Resource Visibility (Client-A write -> Client-B read).
b.
Update Propagation (Client-A update -> immediate Client-B observation).
c.
Uncoordinated Concurrent Update (characterizing current last-writer-wins behavior).
d.
Infinispan Version / Conditional Update (demonstrating replaceWithVersion capabilities experimentally).
e.
Participant Failure (resilience of surviving participants).
f.
Cache Loss / Reconstructability (non-authoritative nature of Mneme state).
g.
Read-Dominant / Jittery Access (~10:1 read/write deterministic workload).
h.
Mneme Unavailable (verifying explicit failure semantics without JVM-local fallback).
i.
Required Persistence Failure (verifying store failure semantics).
j.
Recovery After Mneme Availability Returns (clean reconnection without stale local state).
◦
...
Key Scenarios
1.
Scenario 01 (Distributed Resource Visibility): Client-A writes a resource; Client-B reads and validates identical content over the Hot Rod wire protocol.
2.
Scenario 02 (Update Propagation): Client-A updates existing key; Client-B observes updated value immediately under synchronous replication.
3.
Scenario 03 (Uncoordinated Concurrent Update): Both clients read initial state, then write independent updates; verifies last-writer-wins characterization.
4.
Scenario 04 (Infinispan Version / Conditional Update): Demonstrates replaceWithVersion rejecting stale entry version updates.
5.
Scenario 05 (Participant Failure): Client-A disconnects; Client-B continues reading and writing cluster state without interruption.
6.
Scenario 06 (Cache Loss / Reconstructability): Cache cleared; demonstrates that state is lost from Mneme, confirming non-authoritative boundary.
7.
Scenario 07 (Read-Dominant Access): Deterministic 10:1 read/write workload, proving cache hits without redundant persistence retrieval.
8.
Scenario 08 (Mneme Unavailable): Server stopped; operations fail explicitly with IllegalStateException / HotRodClientException.
9.
Scenario 09 (Persistence Failure): Documents and asserts cache behavior when store write fails.
10.
Scenario 10 (Recovery After Availability Returns): Server restarted; clients reconnect and resume normal operations without stale memory artifacts.
...
Now I'm opening the project plan document to understand the overall goals and context of the distributed behavior laboratory feature before making any code changes.
Requirements
Overview & Goals
Create an executable learning and architecture-verification laboratory within Harmonia that characterizes, explores, and documents the actual distributed behavior of Mneme (Infinispan data grid and Hot Rod clients) in relation to ADR-019. The laboratory serves both as an objective architectural characterization of existing capabilities/gaps and as an educational tool for future developers and architects.
Scope
...
Summary
•
Completed Task 07 Step 04 — BLOCK 2 by creating the minimal real-Infinispan multi-participant test harness and executing Scenarios 01 (Distributed Resource Visibility) and 02 (Update Propagation).
•
Verified runtime distributed availability, synchronous replication (REPL_SYNC), and metadata version progression across independent Hot Rod clients without modifying production code.
Changes
•
Created MnemeScenarioNarrator.java in hestia/mneme-cluster/src/test/java/net/fhirfactory/harmonia/infinispan/scenario/support/ providing structured educational narrative formatting.
•
Created InfinispanLaboratoryServer.java managing two clustered EmbeddedCacheManager nodes (Node-1, Node-2) bound to dynamic HotRodServer socket endpoints, providing independent RemoteCacheManager client instances (Client-A, Client-B).
•
Implemented MnemeDistributedVisibilityScenarioTest.java executing and narrating Scenario 01 and Scenario 02.
•
Updated delivery plan statuses for Step 1 and Step 2.
Verification
•
Executed targeted laboratory suite: mvn test -pl hestia/mneme-cluster -Dtest="MnemeDistributedVisibilityScenarioTest" -Dsurefire.failIfNoSpecifiedTests=false (2/2 passed in ~4.5s).
•
Executed full ArchUnit suite: mvn test -pl paradeigma/paradeigma-test -am -Dtest="*ArchitectureTest" -Dsurefire.failIfNoSpecifiedTests=false (57/57 passed).
Notes
•
Scenarios 03–10 remain pending for subsequent blocks.
•
OperationsAggregatorService.localPragmaStore remains classified as Category B (retained for future remediation decision).