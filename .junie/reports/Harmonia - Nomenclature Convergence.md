I will update the recipient display assertion in IncomingMfnMessageProcessorTest to expect Harmonia MLLP Gateway.

I will compile the entire repository reactor so all downstream modules resolve the updated Calliope classes and enum constants.

I will update IncomingAdtMessageProcessorTest to assert the converged Harmonia-Synthetic-Task reason code.

I will update the main execute wrapper test in OperationsCliCommandTest to expect the mnemosyne-operations-cli binary name.
