I'm inspecting the repository's WildFly provisioning and deployment configurations to determine how to define the `KleioAuditDS` DataSource targeting the FHIR PostgreSQL database (`jdbc:postgresql://postgres-1:5432/fhir_node_1`) without hardcoding credentials in source code.

I'm updating the Step 04.2 plan to specify the explicit `java:jboss/datasources/KleioAuditDS` JNDI binding, configure the `@DataSourceDefinition` in Iris BEFE using external environment variables, add the `datasources` layer to the WildFly Maven plugin, and refine the delivery steps.
...
- **A. Is `java:comp/DefaultDataSource` already configured to target `jdbc:postgresql://postgres-1:5432/fhir_node_1`?**
    - **No.**
- **B. Repository Configuration Analysis**:
    - `iris/iris-befe/pom.xml` configures `wildfly-jar-maven-plugin:11.0.1.Final` with provisioning layers `jaxrs`, `cdi`, `jsonb`, `transactions`, `management` (note: `datasources` layer is not currently provisioned in the bootable JAR plugin configuration).
    - `iris/iris-befe/Dockerfile` builds from `quay.io/wildfly/wildfly:31.0.1.Final-jdk21` where default `standalone.xml` binds `ExampleDS` (H2) as the default DataSource.
    - `deployment/kubernetes/base/iris/iris-befe.yaml` and `docker-compose.yml` provide environment variables for Infinispan and OIDC, but currently declare no database connection configuration for Iris BEFE.
    - `docs/middleware/wildfly.md` (Section 3) explicitly documents Iris Presentation Decoupling: zero direct JPA/Hibernate, zero direct database access, relying on Hot Rod for caching.
- **C. Smallest Composition-Root / Runtime Configuration**:
    - In Iris BEFE source code, `AuditDataSourceProducer` consumes `@Resource(lookup = "java:comp/DefaultDataSource")` (standard Jakarta EE default DataSource binding).
    - In the container runtime environment (WildFly bootable JAR / `standalone.xml` / deployment descriptor), the container binds the PostgreSQL DataSource targeting `jdbc:postgresql://postgres-1:5432/fhir_node_1` to `java:comp/DefaultDataSource` (or an application-defined `@DataSourceDefinition` in the web application).
    - *Deployment Configuration Requirement (Grounding)*: Since BEFE runtime currently has no pre-existing PostgreSQL datasource definition, runtime deployment requires binding `java:comp/DefaultDataSource` to `jdbc:postgresql://postgres-1:5432/fhir_node_1` (`fhir_user`/`fhir_password`).
- **D. Exact JNDI Name for `AuditDataSourceProducer`**:
    - `java:comp/DefaultDataSource` (standard Jakarta EE platform default DataSource JNDI lookup).
...
1. **DataSource Ownership & Semantic Separation**:
    ...
    - `iris-befe` (the composition root) owns the physical binding by providing a `@Produces @KleioAudit @ApplicationScoped DataSource` method that injects `@Resource(lookup = "java:comp/DefaultDataSource")`.
    - *Rationale*: Maintains loose coupling and ensures the physical database target can be relocated in future tasks (e.g. AUDIT-BL-02) without changing Kleio persistence code.
...
graph TD
    subgraph IrisBEFE [Iris BEFE Composition Root]
        ...
        DEFDS[Resource: java:comp/DefaultDataSource]
        DEFDS -->|injected into| ADSP
    end
...
2. **WildFly Runtime / Container Verification**: Validating WAR packaging and deployment readiness under WildFly's JTA and `@Resource` container-managed lifecycle.
...
**Step 1: Configure Maven dependencies and implement the Iris BEFE Kleio Audit DataSource producer**  
Add the `kleio-persistence` dependency to `iris/iris-befe/pom.xml` and create the CDI producer `AuditDataSourceProducer` in Iris BEFE to bind the container-managed DataSource to the `@KleioAudit` qualifier.
...
- Create `iris/iris-befe/src/main/java/net/fhirfactory/harmonia/befe/config/AuditDataSourceProducer.java` annotated with `@ApplicationScoped`.
- Inject the container-managed DataSource via `@Resource(lookup = "java:comp/DefaultDataSource")`.
...
- Ensure Iris BEFE source code imports only `javax.sql.DataSource` and `net.fhirfactory.harmonia.kleio.persistence.qualifier.KleioAudit`, avoiding direct imports of PostgreSQL driver, JPA, Hibernate, or repository internal classes.
...
Now I'm reviewing the iris-befe.yaml file in the deployment configuration to understand its current setup and parameters, so I can determine how to adjust or extend the deployment as needed.

I will now update Iris BEFE's Maven configuration and deployment manifests to incorporate the Kleio persistence dependency, the WildFly datasources layer, and the database environment variables, followed by creating the AuditDataSourceProducer.

All configuration and source files for the DataSource producer and deployment manifests have been implemented and verified. I am now proceeding to Step 2 to implement the CDI composition tests and verify the architecture guardrails.
...
I will now create KleioRuntimeCompositionTest in Iris BEFE to validate unambiguous CDI bean resolution, append/find operations, and container DataSource producer annotations.

All composition and architecture verification tests have executed cleanly with zero violations. I will now update the plan tracking file to mark Step 2 as completed and submit the final summary.
...
**Summary**
- Composed the durable Kleio audit persistence subsystem into the Iris BEFE runtime container as a container-managed CDI service graph, maximizing system reliability and audit record durability.
- Maintained presentation decoupling and clean module boundaries by routing persistence dependencies through qualified `@KleioAudit DataSource` abstractions without direct driver leakage.

**Changes**
- Updated `iris/iris-befe/pom.xml` to declare compile dependency `net.fhirfactory.harmonia:kleio-persistence`, add the `datasources` layer in `wildfly-jar-maven-plugin`, and supply test dependencies.
- Implemented `AuditDataSourceProducer` in Iris BEFE with `@DataSourceDefinition` targeting `java:jboss/datasources/KleioAuditDS` and producing an `@ApplicationScoped @KleioAudit DataSource`.
- Configured database environment variables (`FHIR_DB_URL`, `FHIR_DB_USER`, `FHIR_DB_PASSWORD`) in `deployment/kubernetes/base/iris/iris-befe.yaml` and `docker-compose.yml`.
- Created `KleioRuntimeCompositionTest` verifying unambiguous CDI resolution of `AuditService`, `DurableAuditService`, `AppendOnlyAuditEventRepository`, `FhirContext`, and append/read persistence operations.

**Verification**
- Executed `mvn test -pl kleio/kleio-persistence,iris/iris-befe -am` confirming all 123 tests in Iris BEFE and Kleio persistence passed with zero failures.
- Executed `mvn test -pl paradeigma/paradeigma-test -am -Dtest="*ArchitectureTest"` confirming 100% compliance across `IrisDecouplingArchitectureTest`, `PackageLayeringArchitectureTest`, and `SecurityEnforcementArchitectureTest`.
