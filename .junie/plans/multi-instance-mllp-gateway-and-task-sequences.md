...
The goal is to:
1. Implement a new activity class `PatientDemographicsUpdateErgon` in package `net.fhirfactory.hie.taskprocessors.patient.demographics` extending `ErgonBase`, capable of extracting demographic information (names, DOB, gender, marital status, language, race, ethnicity, contacts/next-of-kin, addresses, telecoms, general practitioners, managing organizations) from any incoming ADT event trigger based message and updating the corresponding Patient resource within the HIE.
2. Add `PatientDemographicsUpdateErgon` to the same `Praxis` as `PatientIdentityUpdateErgon` (`seq-patient-identity-pipeline`), ensuring that `PatientDemographicsUpdateErgon` executes AFTER `PatientIdentityUpdateErgon`.

### Scope
- **In Scope**:
  - Implementation of `PatientDemographicsUpdateErgon` in `workflow-services/task-processors` under `net.fhirfactory.hie.taskprocessors.patient.demographics`.
  ...
  - Merging and updating existing FHIR `Patient` resources produced by upstream activities (like `PatientIdentityUpdateErgon`) without losing identity or demographic data.
  - Integration with `TaskSequenceLoader` to include `PatientDemographicsUpdateErgon` in `seq-patient-identity-pipeline` sequentially following `PatientIdentityUpdateErgon`.
...
- Create `PatientDemographicsUpdate.java` in `net.fhirfactory.hie.taskprocessors.patient.demographics` extending `ErgonBase`.
...
- Create `PatientDemographicsUpdateErgonTest` in `task-processors`.
- Update `TaskSequenceLoaderTest` and `TaskSequenceTest` to verify sequence chaining (`PatientIdentityUpdateErgon` -> `PatientDemographicsUpdateErgon`).
