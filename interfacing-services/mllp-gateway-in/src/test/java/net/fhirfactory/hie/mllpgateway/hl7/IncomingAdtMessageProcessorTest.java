...
class IncomingAdtMessageProcessorTest {
    ...
    void setUp() {
        ...
        transformer = new IncomingAdtMessageProcessor(taskService, communicationService, provenanceService, taskEventProducerService);
    }
    ...
    void testProcessAdtA01Message() {
        ...
        // Verify Task links to source Communication via basedOn
        ...
        // Verify Provenance resource created and linked
        Provenance prov = result.getProvenance();
        assertThat(prov).isNotNull();
        assertThat(prov.getIdPart()).isEqualTo("prov-MSG-A01-001");
        assertThat(prov.getTarget()).hasSize(1);
        assertThat(prov.getTargetFirstRep().getReference()).isEqualTo("Task/MSG-A01-001");
        assertThat(prov.getTargetFirstRep().getDisplay()).contains("JOHN SMITH");

        // Verify Provenance entity links to source Communication
        assertThat(prov.getEntity()).hasSize(1);
        Provenance.ProvenanceEntityComponent provEntity = prov.getEntityFirstRep();
        assertThat(provEntity.getRole()).isEqualTo(Provenance.ProvenanceEntityRole.SOURCE);
        assertThat(provEntity.getWhat().getReference()).isEqualTo("Communication/comm-MSG-A01-001");

        // Verify Provenance patient context, activity, agents and location
        assertThat(prov.getPatient().getReference()).isEqualTo("Patient/PAT10099");
        assertThat(prov.getActivity().getCodingFirstRep().getCode()).isEqualTo("CREATE");
        assertThat(prov.getAgent()).hasSize(3); // Transmitter, Assembler, Author (Dr. Jones)
        assertThat(prov.getLocation().getDisplay()).contains("WARDA");

        // Verify Provenance was persisted in ProvenanceService
        assertThat(provenanceService.count()).isEqualTo(1);
        assertThat(provenanceService.getById("prov-MSG-A01-001")).isPresent();
        ...
        try {
            ...
            TaskEvent sentEvent = eventCaptor.getValue();
    ...
    void testProcessAdtA03DischargeMessage() {
        ...
        Bundle bundle = (Bundle) result.getTask().getContained().get(0);
        Encounter encounter = (Encounter) bundle.getEntry().stream()
    ...
    void testProcessAdtA08UpdateMessageWithEmergency() {
        ...
        assertThat(patient.getNameFirstRep().getGivenAsSingleString()).isEqualTo("ALICE");
    }
    ...
    void testProcessAdtWithMultipleNextOfKinAndPractitioners() {
        ...
        Task task = result.getTask();
        Bundle bundle = (Bundle) task.getContained().get(0);
        List<Bundle.BundleEntryComponent> entries = bundle.getEntry();

        // Check entries: 1 Patient + 2 RelatedPersons + 3 Practitioners + 1 Encounter = 7 entries
        assertThat(entries).hasSize(7);

        long relatedPersonCount = entries.stream().filter(e -> e.getResource() instanceof RelatedPerson).count();
        assertThat(relatedPersonCount).isEqualTo(2);

        long practitionerCount = entries.stream().filter(e -> e.getResource() instanceof Practitioner).count();
        assertThat(practitionerCount).isEqualTo(3);

        Encounter enc = (Encounter) entries.stream().filter(e -> e.getResource() instanceof Encounter).findFirst().orElseThrow().getResource();
        assertThat(enc.getParticipant()).hasSize(3);
    }

    @Test
    void testCommunicationPersistedBeforeTaskAndProvenancePersistedAfterTask() {
        ...
        DefaultProvenanceService mockProvService = mock(DefaultProvenanceService.class);
        ...
        when(mockProvService.create(any(Provenance.class))).thenAnswer(invocation -> invocation.getArgument(0));

        IncomingAdtMessageProcessor testTransformer = new IncomingAdtMessageProcessor(mockTaskService, mockCommService, mockProvService, taskEventProducerService);
        AdtProcessingResult res = testTransformer.processAdtMessage(hl7);

        assertThat(res.isSuccess()).isTrue();

        org.mockito.InOrder inOrder = inOrder(mockCommService, mockTaskService, mockProvService);
        inOrder.verify(mockCommService).create(any(Communication.class));
        inOrder.verify(mockTaskService).create(any(Task.class));
        inOrder.verify(mockProvService).create(any(Provenance.class));
    }

    @Test
    void testProvenanceCreatedIfAndOnlyIfTaskCreatedSuccessfully() {
        ...
        when(mockCommService.create(any(Communication.class))).thenAnswer(invocation -> invocation.getArgument(0));
        // Simulate TaskService failing to create / push task (returns null)
        ...
        assertThat(res.getProvenance()).isNull();

        // ProvenanceService.create must NOT be called if Task is null
        verify(mockProvService, never()).create(any(Provenance.class));
    }
    ...
    void testProcessNonAdtTriggerTypes() throws Exception {
        ...
        verify(mockProducer).sendTaskEvent(eventCaptor.capture());
        TaskEvent capturedOrmEvent = eventCaptor.getValue();
