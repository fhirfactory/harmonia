...
class MfnModularBuildersTest {
    ...
    void setUp() {
        ...
        this.practitionerBuilder = new MfnPractitionerResourceBuilder(extractor);
        this.administrativeBuilder = new MfnAdministrativeResourceBuilder(extractor);
        ...
        this.provenanceBuilder = new MfnProvenanceResourceBuilder(extractor);
    }
    ...
    @Test
    @DisplayName("Test Practitioner and PractitionerRole resource construction")
    void testPractitionerAndRoleResourceBuilders() throws Exception {
        Message msg = pipeParser.parse(FULL_MFN_M02);
        Terser terser = new Terser(msg);

        String practId = extractor.extractPractitionerId(FULL_MFN_M02, terser);
        assertThat(practId).isEqualTo("DOC-12345");

        List<Practitioner> practitioners = practitionerBuilder.buildPractitioners(terser, FULL_MFN_M02, "MSG-CTRL-99901");
        assertThat(practitioners).hasSize(1);
        Practitioner p = practitioners.get(0);
        assertThat(p.getId()).isEqualTo("Practitioner/DOC-12345");
        assertThat(p.getActive()).isTrue();
        assertThat(p.getNameFirstRep().getFamily()).isEqualTo("SMITH");
        assertThat(p.getNameFirstRep().getGivenAsSingleString()).isEqualTo("JOHN ROBERT");
        assertThat(p.getGender()).isEqualTo(Enumerations.AdministrativeGender.MALE);
        assertThat(p.getTelecom()).isNotEmpty();
        assertThat(p.getAddress()).isNotEmpty();
        assertThat(p.getQualification()).isNotEmpty();

        List<Organization> orgs = administrativeBuilder.buildOrganizations(terser, FULL_MFN_M02, "HOSPITAL_A", "HIE_DEST");
        List<Location> locs = administrativeBuilder.buildLocations(terser, FULL_MFN_M02);
    ...
    @Test
    @DisplayName("Test Administrative resource construction (Organization, Location)")
    void testAdministrativeResourceBuilder() throws Exception {
        Message msg = pipeParser.parse(FULL_MFN_M02);
        Terser terser = new Terser(msg);
    ...
    @Test
    @DisplayName("Test Bundle resource builder")
    void testBundleResourceBuilder() throws Exception {
        Message msg = pipeParser.parse(FULL_MFN_M02);
        Terser terser = new Terser(msg);

        List<Organization> orgs = administrativeBuilder.buildOrganizations(terser, FULL_MFN_M02, "HOSPITAL_A", "HIE_DEST");
        List<Location> locs = administrativeBuilder.buildLocations(terser, FULL_MFN_M02);
        List<Practitioner> practs = practitionerBuilder.buildPractitioners(terser, FULL_MFN_M02, "MSG-CTRL-99901");
        List<PractitionerRole> roles = practitionerBuilder.buildPractitionerRoles(terser, FULL_MFN_M02, practs, orgs, locs);

        Bundle bundle = bundleBuilder.buildBundle("MSG-CTRL-99901", orgs, locs, practs, roles);
    ...
    @Test
    @DisplayName("Test Communication, Task, and Provenance resource builders")
    void testCommunicationTaskAndProvenanceBuilders() throws Exception {
        Message msg = pipeParser.parse(FULL_MFN_M02);
        Terser terser = new Terser(msg);
        ...
        assertThat(task.getContained()).isNotEmpty();

        Provenance prov = provenanceBuilder.buildProvenance(terser, topic, "MSG-CTRL-99901",
                "DOC-12345", "DR. JOHN SMITH", "STAFF_APP", "HOSPITAL_A", "20260910120000", task, comm);
        assertThat(prov.getId()).isEqualTo("Provenance/prov-MSG-CTRL-99901");
        assertThat(prov.getTargetFirstRep().getReference()).isEqualTo("Task/MSG-CTRL-99901");
        assertThat(prov.getEntityFirstRep().getWhat().getReference()).isEqualTo("Communication/comm-MSG-CTRL-99901");
    }
}
