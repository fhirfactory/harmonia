...
/**
 * Builds FHIR {@link Provenance} resources linking the generated Task and source Communication.
 */
@ApplicationScoped
public class AdtProvenanceResourceBuilder {

    private final AdtMessageExtractor extractor;

    public AdtProvenanceResourceBuilder() {
        this(new AdtMessageExtractor());
    }

    @Inject
    public AdtProvenanceResourceBuilder(AdtMessageExtractor extractor) {
    ...
    public Provenance buildProvenance(Terser terser, Topic topic, String messageControlId,
        ...
        return buildProvenance(terser, topic, messageControlId, triggerEvent, patientId, patientFullName,
                sendingApp, sendingFacility, messageTimestamp, assignedLocationPointOfCare,
                attendingDocId, attendingDocName, savedTask, savedCommunication);
    }
    ...
    public Provenance buildProvenance(Terser terser, String messageControlId, String triggerEvent,
                                      ...
                                      String assignedLocationPointOfCare, String attendingDocId, String attendingDocName,
                                      Task savedTask, Communication savedCommunication) {
        return buildProvenance(terser, null, messageControlId, triggerEvent, patientId, patientFullName,
                sendingApp, sendingFacility, messageTimestamp, assignedLocationPointOfCare,
                attendingDocId, attendingDocName, savedTask, savedCommunication);
    }
    ...
    public Provenance buildProvenance(Terser terser, Topic topic, String messageControlId, String triggerEvent,
        ...
        if (savedTask != null) {
            ...
            if (StringUtils.isNotBlank(taskId)) {
                ...
                targetRef.setDisplay("Task for " + StringUtils.defaultIfBlank(patientFullName, "Patient") + " (HL7 ADT " + triggerEvent + ")");
        ...
        // 2. Activity: HL7 ADT Message Transformation
        ...
        activity.addCoding(new Coding("http://example.org/hl7/trigger-event", triggerEvent, "HL7 ADT " + triggerEvent));
        activity.setText("Ingestion and Transformation of HL7 ADT " + triggerEvent + " Event");
        ...
        if (savedCommunication != null) {
            ...
            sourceEntity.setWhat(new Reference(commId).setDisplay("Raw HL7 ADT Message Communication"));
        }
        ...
        if (StringUtils.isNotBlank(patientId) && !"UNKNOWN".equalsIgnoreCase(patientId)) {
            ...
            provenance.setPatient(patientRef);
        }
        ...
        // Agent C: Attending Doctor (Author / Responsible Clinician, if present)
        if (StringUtils.isNotBlank(attendingDocId)) {
            Provenance.ProvenanceAgentComponent docAgent = provenance.addAgent();
            docAgent.setType(new CodeableConcept().addCoding(
                    new Coding("http://terminology.hl7.org/CodeSystem/provenance-participant-type", "author", "Author")));
            Reference docRef = new Reference("Practitioner/" + attendingDocId);
            if (StringUtils.isNotBlank(attendingDocName)) {
                docRef.setDisplay(attendingDocName);
            }
            docAgent.setWho(docRef);
        }

        // 7. Location Context
        if (StringUtils.isNotBlank(assignedLocationPointOfCare)) {
            provenance.setLocation(new Reference().setDisplay("Assigned Point of Care: " + assignedLocationPointOfCare));
        }
