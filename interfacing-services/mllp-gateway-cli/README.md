# HIE MLLP Gateway CLI

A high-performance command-line utility for generating, customizing, and transmitting HL7 v2.4 ADT (Admission, Discharge, Transfer) messages over Minimal Lower Layer Protocol (MLLP) to the HIE MLLP Gateway (`mllp-gateway`).
...
The `mllp-gateway-cli` serves as the primary testing, verification, and synthetic data ingestion client for the HIE MLLP Gateway subsystem. It enables engineers and system integrators to:

- **Verify Integration**: Test end-to-end processing pipelines (MLLP Gateway -> Camel Routes -> FHIR Transformer -> Infinispan Cache / JPA Store).
...
```mermaid
sequenceDiagram
    participant CLI as mllp-gateway-cli
    participant GW as MLLP Gateway (Port 2575)
    participant HIE as HIE Core Platform

    CLI->>GW: <VT> MSH|^~\&|... \r PID|... \r PV1|... \r <FS><CR>
    Note over GW: Parse HL7 v2.4, create FHIR Task & Resources
    GW->>HIE: Persist FHIR Task / Provenance / Communication
...
```bash
# From project root:
mvn clean package -pl interfacing-services/mllp-gateway-cli -DskipTests

# Or from inside interfacing-services/mllp-gateway-cli:
cd interfacing-services/mllp-gateway-cli
mvn clean package -DskipTests
```

The build produces a self-contained executable JAR located at:
```
interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar
```
...
```bash
alias mllp-cli="java -jar $(pwd)/interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar"
```
...
```bash
java -jar interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar \
  -t A01 \
  -m PAT10001 \
  -f JANE \
  -l DOE \
...
```bash
java -jar interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar --list-templates
```
...
java -jar interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar -t A01
```
...
```bash
java -jar interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar -t A01 -v
```
...
JAR="interfacing-services/mllp-gateway-cli/target/mllp-gateway-cli-1.0.0-SNAPSHOT.jar"
