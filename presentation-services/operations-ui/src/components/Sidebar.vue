...
import { 
  ...
  Radio, HardDrive, ChevronLeft, ChevronRight, 
  Search, X, ExternalLink, Sparkles, Server, Network
...
const navSections: NavSection[] = [
  {
    ...
    items: [
      { 
        ...
        description: '5-Tier cluster architecture & live topology status' 
      }
    ]
  },
  {
    ...
    items: [
      { 
        ...
        icon: GitMerge, 
        badge: 'Pipelines', 
        description: 'Sequential activity chains & trigger filters' 
      },
      { 
        name: 'Operational Data', 
        path: '/operations-data', 
        icon: Database, 
        description: 'Non-FHIR relational JPA persistence records' 
      }
    ]
  },
  {
    ...
    items: [
      { 
        ...
        path: '/queues', 
        icon: Radio, 
        description: 'ActiveMQ Artemis queues & live event dispatcher' 
      },
      { 
        name: 'Cache Cluster', 
        path: '/caches', 
        icon: HardDrive, 
        description: 'Infinispan replicated caches & Hot Rod grid' 
      }
    ]
  }
];
    ...
    >
        ...
        <router-link to="/" class="sidebar-brand" @click="handleNavClick">
          ...
          <div v-if="!collapsed" class="sidebar-brand-text">
            <div class="sidebar-brand-title">
              ...
              <span class="badge badge-green text-[10px] py-0 px-1.5">5-Tier</span>
            </div>
            <span class="sidebar-brand-sub">Cluster &amp; Sequences Hub</span>
          </div>
        </router-link>
          ...
          <div class="sidebar-menu">
            ...
            >
              ...
              <span 
                ...
                class="badge badge-green text-[9px] py-0 px-1 ml-auto"
              >
                {{ item.badge }}
              </span>
              ...
              <div v-if="collapsed" class="sidebar-tooltip">
                <div class="font-semibold text-emerald-400">{{ item.name }}</div>
      ...
      <div class="sidebar-footer">
        <a 
          ...
          :title="collapsed ? 'Switch to FHIR Resource Explorer (Port 3000)' : ''"
        >
          <div class="flex items-center gap-2">
            <Sparkles :size="16" class="text-sky-400 flex-shrink-0" />
            <span v-if="!collapsed" class="text-xs font-semibold text-slate-200">FHIR Explorer</span>
          </div>
          <ExternalLink v-if="!collapsed" :size="13" class="text-slate-400" />
          
          <div v-if="collapsed" class="sidebar-tooltip">
            <div class="font-semibold text-sky-400">FHIR Explorer</div>
            <div class="text-[11px] text-slate-400 font-normal">Switch to FHIR R5 Resource Repository</div>
          </div>
        </a>

        <div v-if="!collapsed" class="sidebar-system-info">
          <div class="flex items-center justify-between text-[11px] text-slate-500">
            <span>Ops Center v1.0</span>
...
.sidebar {
  ...
  background: linear-gradient(180deg, #111827 0%, #0c1421 100%);
