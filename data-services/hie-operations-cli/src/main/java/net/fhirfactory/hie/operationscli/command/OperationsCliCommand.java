...
package net.fhirfactory.hie.operationscli.command;

import com.fasterxml.jackson.databind.ObjectMapper;
import net.fhirfactory.hie.model.queue.MessageQueueDefinition;
import net.fhirfactory.hie.operationscli.client.OperationsHttpClient;
import net.fhirfactory.hie.operationscli.formatter.OutputFormatter;
...
public class OperationsCliCommand implements Callable<Integer> {
    ...
    private int executeListQueues(OperationsHttpClient client) throws Exception {
        ...
        List<MessageQueueDefinition> queues = client.listMessageQueues();
    ...
    private int executeGetQueue(OperationsHttpClient client, String queueId) throws Exception {
        ...
        Optional<MessageQueueDefinition> qOpt = client.getMessageQueue(queueId);
        if (qOpt.isPresent()) {
    ...
    private int executeCreateQueue(OperationsHttpClient client) throws Exception {
        MessageQueueDefinition qDef;

        if ((jsonData != null && !jsonData.isBlank()) || file != null) {
            String payload = loadPayload();
            qDef = client.getObjectMapper().readValue(payload, MessageQueueDefinition.class);
        } else {
            ...
            qDef = new MessageQueueDefinition();
    ...
    private int executeCreateSequence(OperationsHttpClient client) throws Exception {
        ...
        if ((jsonData != null && !jsonData.isBlank()) || file != null) {
            String payload = loadPayload();
            seq = client.getObjectMapper().readValue(payload, TaskSequenceDto.class);
        } else {
            ...
            seq = new TaskSequenceDto();
            seq.setSequenceId(sequenceId);
            seq.setSequenceName(effectiveName);
        ...
        if (verbose) {
            out.println("Saving TaskSequence '" + seq.getSequenceId() + "' to " + client.getServerUrl() + "...");
        }

        String result = client.saveTaskSequence(seq);
        out.println("Successfully created/saved TaskSequence: " + seq.getSequenceId());
        if (verbose || "json".equalsIgnoreCase(outputFormat)) {
            out.println(formatter.formatJsonString(result, pretty));
        }
        return 0;
    ...
    private int executeInitConfig(OperationsHttpClient client) throws Exception {
        ...
        for (MessageQueueDefinition q : res.getQueues()) {
            out.println("      * " + q.getQueueName() + " (" + q.getRoutingType() + ")");
        }
        out.println("  - Task Sequences populated: " + res.getSequenceCount());
        for (TaskSequenceDto s : res.getSequences()) {
            out.println("      * " + s.getSequenceId() + " (" + s.getSequenceName() + ")");
        }
        return 0;
    }
    ...
    private int executeListSequences(OperationsHttpClient client) throws Exception {
        ...
        List<TaskSequenceDto> sequences = client.listTaskSequences();
    ...
    private int executeGetSequence(OperationsHttpClient client, String sequenceId) throws Exception {
        ...
        Optional<TaskSequenceDto> seqOpt = client.getTaskSequence(sequenceId);
        if (seqOpt.isPresent()) {
    ...
    public static class SaveSequenceCmd implements Callable<Integer> {
        ...
        public Integer call() {
            try {
                ...
                TaskSequenceDto dto = mapper.readValue(payload, TaskSequenceDto.class);
                if (sequenceId != null && !sequenceId.isBlank()) {
                ...
                parent.out.println("Successfully saved TaskSequence: " + dto.getSequenceId());
                if (parent.verbose) {
    ...
    public static class SaveQueueCmd implements Callable<Integer> {
        ...
        public Integer call() {
            try {
                ...
                ObjectMapper mapper = parent.getHttpClient().getObjectMapper();
                MessageQueueDefinition dto = mapper.readValue(payload, MessageQueueDefinition.class);
                if (queueId != null && !queueId.isBlank()) {
