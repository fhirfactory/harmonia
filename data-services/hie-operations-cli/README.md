# HIE Operations CLI (`hie-operations-cli`)

A command-line tool for interacting with the **HIE Operations JPA Server** (`hie-operations-jpa-server`), inspecting non-FHIR operational resources, and managing **TaskSequence** definitions across the HIE platform.
...
The CLI communicates with the REST endpoints exposed by `hie-operations-jpa-server`:
...
```bash
mvn clean package -pl data-services/hie-operations-cli -am
```

The build produces a self-contained shaded executable JAR at:
```
data-services/hie-operations-cli/target/hie-operations-cli-1.0.0-SNAPSHOT.jar
```
