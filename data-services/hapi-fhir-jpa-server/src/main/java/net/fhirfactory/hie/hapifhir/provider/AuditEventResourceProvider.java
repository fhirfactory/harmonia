...
public class AuditEventResourceProvider implements IResourceProvider {
    ...
    public AuditEventResourceProvider(FhirStorageService storageService) {
    ...
    @Create
    public MethodOutcome create(@ResourceParam AuditEvent resource) {
        AuditEvent created = storageService.createResource(resource);
    ...
    public AuditEvent read(@IdParam IdType theId) {
        return storageService.getResource("AuditEvent", theId.getIdPart());
    }

    @Update
    public MethodOutcome update(@IdParam IdType theId, @ResourceParam AuditEvent resource) {
        AuditEvent updated = storageService.updateResource(theId.getIdPart(), resource);
        MethodOutcome outcome = new MethodOutcome();
    ...
    public MethodOutcome delete(@IdParam IdType theId) {
        storageService.deleteResource("AuditEvent", theId.getIdPart());
        MethodOutcome outcome = new MethodOutcome();
        outcome.setId(theId);
        return outcome;
    }
    ...
    public List<AuditEvent> search(
            ...
            @Description(shortDefinition = "An action or agent name")
            @OptionalParam(name = "name") StringParam theName,
            @Description(shortDefinition = "An audit event identifier")
        ...
        return storageService.searchResources("AuditEvent", idStr, nameStr, identStr);
    }
}
