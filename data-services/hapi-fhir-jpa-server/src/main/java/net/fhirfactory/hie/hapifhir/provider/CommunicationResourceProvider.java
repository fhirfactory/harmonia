...
public class CommunicationResourceProvider implements IResourceProvider {
    ...
    public CommunicationResourceProvider(FhirStorageService storageService) {
    ...
    @Create
    public MethodOutcome create(@ResourceParam Communication resource) {
        Communication created = storageService.createResource(resource);
    ...
    public Communication read(@IdParam IdType theId) {
        return storageService.getResource("Communication", theId.getIdPart());
    }

    @Update
    public MethodOutcome update(@IdParam IdType theId, @ResourceParam Communication resource) {
        Communication updated = storageService.updateResource(theId.getIdPart(), resource);
        MethodOutcome outcome = new MethodOutcome();
    ...
    public MethodOutcome delete(@IdParam IdType theId) {
        storageService.deleteResource("Communication", theId.getIdPart());
        MethodOutcome outcome = new MethodOutcome();
        outcome.setId(theId);
        return outcome;
    }
    ...
    public List<Communication> search(
            ...
            @Description(shortDefinition = "A subject or note description")
            @OptionalParam(name = "name") StringParam theName,
            @Description(shortDefinition = "A communication identifier")
        ...
        return storageService.searchResources("Communication", idStr, nameStr, identStr);
    }
}
