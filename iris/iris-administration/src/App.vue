...
onMounted(() => {
  const saved = localStorage.getItem('iris-admin-sidebar-collapsed');
  if (saved !== null) {
...
const toggleCollapse = () => {
  ...
  localStorage.setItem('iris-admin-sidebar-collapsed', String(isSidebarCollapsed.value));
};
...
<template>
  <div class="app-root min-h-screen flex bg-[#0b0f19] text-slate-100 font-sans">
    <!-- Collapsible Extensible Sidebar -->
    <Sidebar 
    ...
    >
      ...
      <footer class="border-t border-[#1f293d] bg-[#111827]/60 py-4 px-6 text-center md:text-left text-xs text-slate-500">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2">
          <div>
            <span class="font-bold text-slate-300">Harmonia Iris Administration &amp; Self-Service</span>
            <span class="mx-2">&bull;</span>
            <span>FHIR R5 Provider Registry Management Console</span>
          </div>
          <div class="flex items-center gap-3 text-slate-400">
            <span>Themis ABAC</span>
            <span>&bull;</span>
            <span>BEFE (Port 8080)</span>
            <span>&bull;</span>
            <span>Mnemosyne Storage</span>
          </div>
        </div>
      </footer>
    </div>
