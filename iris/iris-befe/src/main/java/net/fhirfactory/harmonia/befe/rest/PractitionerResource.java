...
public class PractitionerResource {
    ...
    public Response search(
        ...
        Bundle bundle = cacheService.searchAsBundle("Practitioner", id, name, identifier);
    ...
    public Response read(@PathParam("id") String id) {
        String json = cacheService.getResourceJson("Practitioner", id);
        if (json == null) {
            return Response.status(Response.Status.NOT_FOUND)
                    .entity("{\"resourceType\":\"OperationOutcome\",\"issue\":[{\"severity\":\"error\",\"code\":\"not-found\",\"diagnostics\":\"Practitioner/" + id + " not found\"}]}")
                    .build();
        }
        return Response.ok(json).build();
    }
    ...
    public Response create(String payload) {
        ...
        Practitioner resource = parser.parseResource(Practitioner.class, payload);
        Practitioner saved = cacheService.saveResource(resource);
        String json = parser.encodeResourceToString(saved);
        return Response.created(URI.create("/api/fhir/Practitioner/" + saved.getIdElement().getIdPart()))
                .entity(json)
                .build();
    }
    ...
    public Response update(@PathParam("id") String id, String payload) {
        ...
        IParser parser = cacheService.getJsonParser();
        Practitioner resource = parser.parseResource(Practitioner.class, payload);
        resource.setId("Practitioner/" + id);
        Practitioner saved = cacheService.saveResource(resource);
        String json = parser.encodeResourceToString(saved);
        return Response.ok(json).build();
    }
    ...
    public Response delete(@PathParam("id") String id) {
        boolean deleted = cacheService.deleteResource("Practitioner", id);
        if (deleted) {
