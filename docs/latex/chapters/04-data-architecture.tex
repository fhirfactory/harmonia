...
Harmonia implements full lifecycle create, read, update, domain lifecycle state transitions, and search capabilities for 15 core FHIR R5 resources (ADR-020):
...
\begin{itemize}
    ...
    \item \textbf{Audit-Preserving Lifecycle State (ADR-020)}: Governed persisted information is never physically deleted. Deactivated or retired resources maintain their historical records and Kleio audit evidence with \texttt{is\_deleted = TRUE} via authoritative \texttt{UPDATE} operations.
\end{itemize}
