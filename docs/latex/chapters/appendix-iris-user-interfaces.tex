...
The console is architected around six canonical operational perspectives and a diagnostic event flow, unified by the shared \texttt{@harmonia/iris-befe} design system and Aura Light theme:

\begin{figure}[htbp]
\centering
\begin{archinote}
\textbf{Persistent Shell Architecture (\texttt{IrisApplicationShell.vue})}:
\begin{itemize}
    \item \textbf{Global Operations Header (\texttt{IrisHeader.vue})}: Displays active deployment environment (e.g. \texttt{PROD / microk8s-01}), cluster status, platform overall health badge (\texttt{HEALTHY}, \texttt{DEGRADED}, \texttt{UNAVAILABLE}), active subsystem count, critical and warning alert pill indicators, current timestamp, last refresh timestamp, and manual refresh trigger.
    \item \textbf{Persistent Top Navigation (\texttt{IrisPrimaryNavigation.vue})}: High-contrast top navigation bar with direct routes to the six canonical operational perspectives: \texttt{/overview}, \texttt{/subsystems}, \texttt{/interfaces}, \texttt{/work}, \texttt{/messages}, and \texttt{/health}.
    \item \textbf{Standardized Page Action Bar (\texttt{IrisToolbar.vue})}: Consistent page-level action bar supporting search filtering, time-window selectors (\texttt{15m}, \texttt{1h}, \texttt{6h}, \texttt{24h}), manual refresh with spin indicator, and action triggers.
\end{itemize}
\end{archinote}
...
\subsection{The Six Canonical Operational Perspectives}

\begin{enumerate}
    \item \textbf{OVERVIEW Perspective (\texttt{/overview})}:
    Answers \textit{"Is Harmonia healthy? Is work moving? Are messages backing up?"}
    Features a high-density platform health strip, active alert ribbons with direct links to degraded subsystems, active work execution rates, and an area-by-area summary across all six Harmonia architectural areas.

    \item \textbf{SUBSYSTEMS Perspective (\texttt{/subsystems})}:
    Answers \textit{"What is the authoritative architectural status?"}
    Operates as the primary proving ground for the Iris Design System via a Split-Tree Master-Detail layout:
    \begin{itemize}
        \item \textbf{Architectural Tree Navigator (\texttt{SubsystemTreeNavigator.vue} / \texttt{IrisHierarchy})}: Organizes Harmonia into the six authoritative architectural areas: \textit{Integration \& Transport} (Pylai, Petasos), \textit{Execution \& Processing} (Energeia: Ponos, Ergon, Praxis), \textit{Information \& State} (Calliope, Mneme, Mnemosyne), \textit{Security \& Policy} (Themis), \textit{Collaboration} (Agora), and \textit{Presentation} (Iris). Displays accessible status badges and mandatory English subtitles alongside Greek mythological names.
        \item \textbf{Subsystem Header (\texttt{IrisSubsystemIdentity.vue})}: Displays Greek name, English architectural description, version, status badge, and action buttons.
        \item \textbf{Master-Detail Tabbed Inspection}: Features three tabs: \textit{Runtime Instances} (using \texttt{IrisDataTable} with compact row density and slide-out Pod detail drawer), \textit{Operational Health \& Dependencies} (reporting P95 latency and dependency round-trip latencies), and \textit{Telemetry Statistics} (reactive SVG micro-charts styled with Iris light theme tokens).
        \item \textbf{Middleware Subordination}: Subordinates runtime infrastructure under consuming Harmonia capabilities (ActiveMQ Artemis under Petasos, Infinispan under Mneme, PostgreSQL under Mnemosyne, Synapse under Agora).
    \end{itemize}

    \item \textbf{INTERFACES Perspective (\texttt{/interfaces})}:
    Answers \textit{"Are protocol gateways listening and communicating?"}
    Compact operational table of Pylai inbound and outbound gateways (\texttt{pylai-mllp-in}, \texttt{pylai-mllp-out-his}, \texttt{pylai-mllp-out-lis}, \texttt{pylai-fhir-registry}) displaying direction, protocol, port, live status, and safety policy compliance (Invariant 4 Dual-Write AA ACK safety and Invariant 5 Destination Fan-Out). Where real-time throughput metrics are pending live Camel hooks, honest placeholders are rendered per \texttt{IRIS-API-GAP-001}.

    \item \textbf{WORK Perspective (\texttt{/work}, formerly \texttt{/workflows})}:
    Answers \textit{"What tasks and workflows are currently executing?"}
    High-level indicators of active Praxis executions, Ponos task worker rates, queued/running/completed/failed work, and diagnostic drill-down into concrete Pragma execution envelopes and Ergon checkpoint timelines.

    \item \textbf{MESSAGES Perspective (\texttt{/messages}, formerly \texttt{/queues})}:
    Answers \textit{"How are messaging transport and queues performing?"}
    ...
    \item \textbf{HEALTH \& ALERTS Perspective (\texttt{/health}, \texttt{/alerts})}:
    Answers \textit{"What currently requires operator attention?"}
    Cross-subsystem operational health matrix, dependency latencies, actionable operational alerts classified by severity (\texttt{CRITICAL}, \texttt{WARNING}, \texttt{INFORMATION}) and lifecycle status (\texttt{ACTIVE}, \texttt{ACKNOWLEDGED}, \texttt{RESOLVED}), with operator remediation guidance and one-click acknowledgment actions.

    \item \textbf{EVENTS Diagnostic Flow (\texttt{/events})}:
    Answers \textit{"What happened to a particular interaction?"}
    Cross-subsystem diagnostic sequence tracing with dual Flowchart and Table views, correlation filtering, and zero-PHI technical metadata inspection.
\end{enumerate}

\subsection{Telemetry Gaps Register (\texttt{IRIS-API-GAP})}
Where backend JAX-RS endpoints on Port 8090 do not yet supply full operational telemetry, limitations are formalized in the API Gap Register:
\begin{itemize}
    \item \textbf{\texttt{IRIS-API-GAP-001}}: Live Interface Throughput and Error Metrics (requires live Camel MBean hooks in \texttt{PylaiHealthProvider}).
    \item \textbf{\texttt{IRIS-API-GAP-002}}: Granular Synapse Matrix Collaboration Metrics (Matrix room counts and AS transaction rates from Agora).
    \item \textbf{\texttt{IRIS-API-GAP-003}}: Granular Interface Restart History and Wire Diagnostics.
    \item \textbf{\texttt{IRIS-API-GAP-004}}: Granular Container CPU/Memory Metrics Outside Kubernetes.
    \item \textbf{\texttt{IRIS-API-GAP-005}}: Durable Alert Acknowledgement Persistence.
\end{itemize}
