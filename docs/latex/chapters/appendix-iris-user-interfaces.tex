...
\begin{figure}[htbp]
	...
	\fbox{%
        ...
        {images/harmonia-iris-architecture.png}
    }
...
\addlinespace
\texttt{iris-console} & Vue 3 SPA & Port 3001 & \textbf{Operations \& Site Reliability Engineers}: Five canonical operational perspectives (Subsystems, Queues, Workflows, Events, Alerts), cross-subsystem event sequence tracing, instance detail inspections, and alert remediation. \\
\addlinespace
...
Clinical FHIR transactions and administrative governance workflows operate over standard JAX-RS REST endpoints on Port 8080, while mission-critical systems operations, broker telemetry, cross-subsystem diagnostics, and alert governance execute over a dedicated socket server on Port 8090. Operational spikes or diagnostic queries can never degrade clinical query bandwidth.
\end{principlebox}
...
\begin{itemize}
    ...
    \item \textbf{Dedicated Operations HTTP Server (Port 8090)}: A lightweight, non-blocking socket server managed by \texttt{OperationsServerManager}. It isolates normalized subsystem, queue, workflow, event, and alert telemetry from JAX-RS connection pools and clinical request threads.
\end{itemize}
...
The \textbf{Iris Operations Console} (\texttt{iris/iris-console}) is the systems administration, telemetry surveillance, and workflow diagnostic dashboard operating on Port 3001. Built for Site Reliability Engineers (SREs), integration engineers, and DevOps personnel, it delivers deep real-time observability, message flow tracing, and incident remediation across the Harmonia integration platform.

\subsection{Architectural Perspectives \& Shell Topology}

The console is architected around five primary operational perspectives accessible via a persistent top navigation bar and unified by a global operational header:

\begin{figure}[htbp]
\centering
\begin{archinote}
\textbf{Persistent Shell Architecture}:
\begin{itemize}
    \item \textbf{Global Operations Header (\texttt{GlobalOperationsHeader.vue})}: Displays the active deployment environment (e.g. \texttt{PROD / microk8s-01}), cluster status, platform overall health badge (\texttt{HEALTHY}, \texttt{DEGRADED}, \texttt{UNAVAILABLE}), active subsystem count, critical and warning alert pill indicators, current timestamp, last refresh timestamp, and manual refresh trigger.
    \item \textbf{Persistent Top Navigation (\texttt{NavigationTopBar.vue})}: Persistent navigation bar with direct, bookmarkable routes to the five canonical operational perspectives: \texttt{/subsystems}, \texttt{/queues}, \texttt{/workflows}, \texttt{/events}, and \texttt{/alerts}.
\end{itemize}
\end{archinote}
\caption{Iris Operations Console Shell and Global Navigation Layout}
\label{fig:iris_console_shell}
\end{figure}

\subsection{The Five Canonical Operational Perspectives}

\begin{enumerate}
    \item \textbf{SUBSYSTEMS Perspective (\texttt{/subsystems})}:
    Answers \textit{"Is Harmonia healthy?"}
    \begin{itemize}
        \item \textbf{Subsystem Inventory Sidebar (\texttt{SubsystemSidebar.vue})}: Displays the complete operational inventory of Harmonia's nine core subsystems: \texttt{Pylai}, \texttt{Petasos}, \texttt{Energeia} (with expandable child nodes \texttt{Ponos} and \texttt{Praxis}), \texttt{Mneme}, \texttt{Mnemosyne}, \texttt{Calliope}, \texttt{Themis}, \texttt{Agora}, and \texttt{Iris}, each equipped with accessible status indicators (\texttt{Healthy}, \texttt{Degraded}, \texttt{Unavailable}, \texttt{Unknown}).
        \item \textbf{Subsystem Header (\texttt{SubsystemHeader.vue})}: Shows selected subsystem name, functional description, overall state, runtime instance count, application version, and last telemetry update timestamp.
        \item \textbf{Instances Table \& Detail Drawer (\texttt{InstanceTable.vue}, \texttt{InstanceDetailDrawer.vue})}: Domain-neutral runtime instance table detailing instance ID, role (Primary, Standby, Worker), state, readiness, restart count, uptime, CPU utilization percentage, and memory allocation in megabytes. Selecting an instance opens a slide-out drawer presenting Kubernetes Pod name, namespace, node, IP address, container image, container version, probe status, and recent operational errors.
        \item \textbf{Health \& Dependencies Panel (\texttt{HealthDependenciesPanel.vue})}: Reports service operational health, availability percentage, failed operations count, restart count, P95 round-trip latency, dependency health summary ratio (e.g. \texttt{4 / 4 Healthy}), and an interactive table detailing downstream dependencies with individual round-trip ping latencies.
        \item \textbf{Statistics Panel (\texttt{StatisticsPanel.vue})}: Features time window selectors (\texttt{15m}, \texttt{1h}, \texttt{6h}, \texttt{24h}), key metric summary cards, and reactive, dark-theme native SVG sparkline and time-series trend micro-charts (\texttt{SvgTimeSeriesChart.vue}).
    \end{itemize}

    \item \textbf{QUEUES Perspective (\texttt{/queues})}:
    Answers \textit{"Is work moving?"}
    \begin{itemize}
        \item \textbf{Summary Metrics}: Displays ActiveMQ Artemis broker cluster topology, total queue count, messages in flight, active consumers, and aggregate Dead-Letter Queue (DLQ) depth.
        \item \textbf{Queues Table (\texttt{QueueTable.vue})}: Real-time telemetry table detailing Queue and Address names, operational status, message depth, consumer and producer counts, enqueue and dequeue throughput rates (messages/sec), oldest message age, redelivery counts, DLQ depth, and message expiry count. Supports search and filtering by queue name, capability binding, and health status.
        \item \textbf{Queue Detail Drawer (\texttt{QueueDetailDrawer.vue})}: Slide-out drawer displaying historical message depth sparklines, throughput rates, and consumer thread metadata with zero exposure of message payloads or clinical content.
    \end{itemize}

    \item \textbf{WORKFLOWS Perspective (\texttt{/workflows})}:
    Answers \textit{"What work is Harmonia currently performing?"}
    \begin{itemize}
        \item \textbf{Summary Cards}: High-level indicators of active Praxis executions, active Pragma instances, queued work, running work, completed work, failed work, retrying work, processing rate, and P95 execution duration.
        \item \textbf{Praxis Workflow Table (\texttt{WorkflowTable.vue})}: Catalog of registered Praxis workflow sequence definitions showing active executions, queued, completed, and failed counts, and P95 latency.
        \item \textbf{Pragma Detail Drawer (\texttt{PragmaDetailDrawer.vue})}: Diagnostic drill-down revealing Pragma execution ID, Praxis ID, lifecycle status (\texttt{RUNNING}, \texttt{COMPLETED}, \texttt{FAILED}, \texttt{RETRYING}), started timestamp, duration, current executing Ergon activity, completed Erga count, retry count, Correlation ID, failure reason code, and Ergon checkpoint execution timeline.
    \end{itemize}

    \item \textbf{EVENTS Perspective (\texttt{/events})}:
    Answers \textit{"What happened to a particular interaction?"}
    \begin{itemize}
        \item \textbf{Diagnostic Search \& Filter (\texttt{EventSearchFilter.vue})}: Multi-attribute search supporting Correlation ID, Causation ID, Message ID, Pragma ID, Subsystem, Event Type, Status (\texttt{SUCCESS}, \texttt{WARNING}, \texttt{FAILURE}), and time window ranges.
        \item \textbf{Operational Flowchart Timeline (\texttt{EventTimeline.vue})}: Vertical connected operational flowchart visualizing message progression across platform boundaries (e.g. Pylai MLLP In $\rightarrow$ Petasos Inbound Queue $\rightarrow$ Ponos Ingestion $\rightarrow$ Ergon Validation $\rightarrow$ Mnemosyne FHIR Commit), showing hop sequence numbers, durations, timestamps, and subsystem color-coded badges. Operators can toggle between flowchart and tabular views.
        \item \textbf{Event Detail Drawer (\texttt{EventDetailDrawer.vue})}: Slide-out drawer presenting technical operational metadata, identifier copy utilities, and error diagnostics. Payloads and PHI are strictly omitted.
    \end{itemize}

    \item \textbf{ALERTS Perspective (\texttt{/alerts})}:
    Answers \textit{"What currently requires operator attention?"}
    \begin{itemize}
        \item \textbf{Severity Tabs \& Filters (\texttt{AlertSeverityTabs.vue})}: Multi-dimensional filtering by severity (\texttt{CRITICAL}, \texttt{WARNING}, \texttt{INFORMATION}), lifecycle status (\texttt{ACTIVE}, \texttt{ACKNOWLEDGED}, \texttt{RESOLVED}), and subsystem.
        \item \textbf{Actionable Alerts Table (\texttt{AlertTable.vue})}: Tabular view showing Alert ID, severity badge, subsystem, component, condition description, first/last observed timestamps, duration, related infrastructure resource, and concrete operator remediation guidance.
        \item \textbf{Operator Acknowledge Action}: Allows on-call operators to acknowledge active alerts directly from the UI, updating backend status via \texttt{POST /api/operations/alerts/\{id\}/acknowledge}.
    \end{itemize}
\end{enumerate}

\subsection{Operations Backend REST API Catalog (Port 8090)}

All operational telemetry is served via \texttt{iris-befe} on Port 8090 under the \texttt{/api/operations/*} resource path:
...
\toprule
\textbf{HTTP Endpoint} & \textbf{HTTP Method} & \textbf{Operational Telemetry Resource} \\
\midrule
\texttt{/api/operations/summary} & \texttt{GET} & Overall platform status, environment, cluster ID, subsystem counts, active alert counts. \\
\texttt{/api/operations/subsystems} & \texttt{GET} & Inventory of all 9 core subsystems and their operational states. \\
\texttt{/api/operations/subsystems/\{id\}} & \texttt{GET} & Single subsystem overview, description, instance count, and version. \\
\texttt{/api/operations/subsystems/\{id\}/instances} & \texttt{GET} & Runtime instances, Pod name, node, role, state, uptime, CPU \%, and memory MB. \\
\texttt{/api/operations/subsystems/\{id\}/health} & \texttt{GET} & Availability \%, failed ops, restart count, P95 latency, and dependency latencies. \\
\texttt{/api/operations/subsystems/\{id\}/statistics} & \texttt{GET} & Time-series statistics (events in/out, throughput, latency percentiles) for 15m/1h/6h/24h. \\
\texttt{/api/operations/queues} & \texttt{GET} & ActiveMQ Artemis queues, depths, consumer counts, rates, and DLQ metrics. \\
\texttt{/api/operations/queues/\{id\}} & \texttt{GET} & Single queue details and historical message depth trends. \\
\texttt{/api/operations/workflows} & \texttt{GET} & Praxis workflow sequence executions, throughput rates, and P95 latency. \\
\texttt{/api/operations/workflows/\{id\}} & \texttt{GET} & Single workflow sequence definition and active counts. \\
\texttt{/api/operations/pragmas/\{id\}} & \texttt{GET} & Pragma execution lifecycle details, retry count, and Ergon checkpoints. \\
\texttt{/api/operations/events} & \texttt{GET} & Cross-subsystem interaction diagnostic events with correlation filtering. \\
\texttt{/api/operations/events/\{id\}} & \texttt{GET} & Single diagnostic event details and correlated sequence metadata. \\
\texttt{/api/operations/alerts} & \texttt{GET} & Operational alerts filtered by severity, status, and subsystem. \\
\texttt{/api/operations/alerts/\{id\}/acknowledge} & \texttt{POST} & Acknowledge active operational alert with operator identity payload. \\
\bottomrule
\end{tabularx}
\caption{Operations Backend REST Endpoint Catalog (iris-befe Port 8090)}
\label{tab:operations_rest_endpoints}
\end{table}
...
\begin{itemize}
    ...
    \item \texttt{ops.read} / \texttt{ops.admin}: Telemetry inspection and operational alert governance authority on Port 8090.
\end{itemize}
...
\subsection{Example 2: Operations Engineer Diagnosing Queue Backpressure and Workflow Stalls}

\textbf{Scenario}: Systems Engineer Alex Rivera detects backpressure in the ActiveMQ Artemis queue \texttt{harmonia.mllp.inbound.v2} and uses the Operations Console to diagnose the root cause and identify stalled workflow executions.

\begin{enumerate}
    \item \textbf{Queue Inspection}: Alex opens \texttt{iris-console} (\texttt{http://localhost:3001}), navigates to \textbf{Queues} (\texttt{/queues}), and inspects \texttt{harmonia.mllp.inbound.v2}. The queue detail drawer shows \texttt{depth: 412}, \texttt{consumerCount: 4}, and a historical depth sparkline indicating sustained growth over the past 15 minutes.
    \item \textbf{Workflow Inspection}: Alex navigates to \textbf{Workflows} (\texttt{/workflows}) and selects the \texttt{seq-mllp-inbound-adt} workflow. The workflow summary shows \texttt{activeExecutions: 8}, \texttt{failedWork: 3}, and \texttt{P95 Duration: 2847ms}. Alex drills down into a failed Pragma instance and observes the failure reason code \texttt{ERR-FHIR-VALIDATION-FAILED} at the \texttt{FhirValidationErgon} checkpoint.
    \item \textbf{Dependency Health Check}: Alex navigates to \textbf{Subsystems} (\texttt{/subsystems}), selects \texttt{Energeia}, and inspects the \textbf{Health \& Dependencies} panel. The panel shows that the \texttt{Mnemosyne} dependency has a round-trip latency of 485ms (elevated from the baseline 14ms), indicating that FHIR validation is timing out due to downstream persistence latency.
    \item \textbf{Alert Acknowledgement}: Alex navigates to \textbf{Alerts} (\texttt{/alerts}) and observes a \texttt{WARNING} alert: \textit{``Energeia P95 latency elevated to 2847ms; Mnemosyne dependency latency 485ms.''}. Alex clicks \textbf{Acknowledge} to mark the alert as reviewed and notes the operator guidance: \textit{``Check Mnemosyne PostgreSQL connection pool saturation and consider scaling read replicas.''}
    \item \textbf{Remediation}: Alex contacts the database team to scale Mnemosyne read replicas. Within 5 minutes, the Mnemosyne dependency latency drops to 18ms, the queue depth begins draining, and the alert status transitions to \texttt{RESOLVED}.
\end{enumerate}
