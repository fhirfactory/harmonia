...
\begin{itemize}
    \item 4 ActiveMQ Artemis instances (Primary A/B, Backup A/B).
    \item 2 Infinispan distributed cache grid nodes.
    \item 2 Spring Boot Mnemosyne JPA persistence servers.
    \item 2 PostgreSQL database servers with persistent volumes.
    \item 3 WildFly Jakarta EE 10 application containers (\texttt{pylai-mllp-in}, \texttt{ponos}, \texttt{iris-befe}).
    \item 2 Vue 3 frontend web containers (\texttt{iris-clinical}, \texttt{iris-console}).
\end{itemize}
