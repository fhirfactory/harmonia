...
\begin{figure}[htbp]
    ...
    \begin{adjustbox}{max width=\textwidth}
        \input{diagrams/fig-deployment-ha.tex}
    \end{adjustbox}
...
\begin{itemize}
    ...
    \item 6 Gateway and workflow application containers (\texttt{pylai-mllp-in}, \texttt{pylai-mllp-out-his}, \texttt{pylai-mllp-out-lis}, \texttt{pylai-fhir-registry}, \texttt{ponos}, \texttt{iris-befe}).
...
\subsection{Provider Registry High Availability \& Crash Recovery}
The FHIR Provider Registry subsystem leverages the clustered Petasos messaging fabric and durable Mnemosyne relational storage to guarantee crash resilience:
\begin{itemize}
    \item \textbf{Durable Queue Storage}: Change requests published to \texttt{harmonia.provider.registry.change.request} are written to the ActiveMQ Artemis persistent journal before \texttt{HTTP 202 Accepted} is returned to the client.
    \item \textbf{In-Flight Process Crash Recovery}: If the Ponos container restarts or terminates during Ergon execution, Artemis retains unacknowledged change requests. Upon broker reconnect, the task is redelivered and re-executed idempotently without generating duplicate resources.
    \item \textbf{Stateless REST Gateways}: The \texttt{pylai-fhir-registry} gateway operates completely statelessly. Multiple instances can be deployed behind a round-robin load balancer for seamless scale-out and zero-downtime rolling upgrades.
\end{itemize}
