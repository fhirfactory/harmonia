...
| **Hestia** | `hestia/mnemosyne-operations`| `mnemosyne-operations` | `IMPLEMENTED` | `jar` | Spring Boot 3.2.5, Spring Data JPA | Deployments (`hie-operations-jpa-server-1`, `hie-operations-jpa-server-2`) / `localhost:32000/harmonia/mnemosyne-operations:1.0.0-SNAPSHOT` | `8080` per Deployment | PostgreSQL 16 (`ops_node_1`, `ops_node_2`) |
| **Hestia** | `hestia/hie-operations-cli` | `hie-operations-cli` | `IMPLEMENTED` | `jar` | Java 21 CLI, Picocli / HTTP REST | CLI Utility | N/A | Mnemosyne Operations REST API |
