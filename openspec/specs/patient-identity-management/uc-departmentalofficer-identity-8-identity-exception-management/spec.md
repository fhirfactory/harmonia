# Identity Exception Management

## Purpose

patients can continue through approved alternative processes

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-Identity-8`
- **Source Scope:** Scope - Patient / Related Person Identity
- **Primary Actor:** Data Steward (Exception Management)
- **Supporting Actors:** Provider Directory
- **User Story:** Manage identity exceptions where standard verification cannot be completed so that patients can continue through approved alternative processes.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Identity Exception Management
Harmonia SHALL support **Identity Exception Management** so that patients can continue through approved alternative processes.

#### Scenario: Identity Exception Management - successful outcome
...
- **WHEN** The actor needs to perform the 'Identity Exception Management' activity, or an authorised business process initiates the activity.
- **THEN** Data Steward (Exception Management) initiates the 'Identity Exception Management' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Data Steward (Exception Management) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Identity Exception Management' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Data Steward (Exception Management)
