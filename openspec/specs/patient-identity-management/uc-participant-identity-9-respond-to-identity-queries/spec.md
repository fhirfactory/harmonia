# Respond to Identity Queries

## Purpose

my registration or access is not delayed

## Source Traceability

- **Source Use Case:** `UC-Participant-Identity-9`
- **Source Scope:** Scope - Identity
- **Primary Actor:** Participant / Patient
- **Supporting Actors:** Provider Directory
- **User Story:** Respond to requests for additional identity information so that my registration or access is not delayed.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Respond to Identity Queries
Harmonia SHALL support **Respond to Identity Queries** so that my registration or access is not delayed.

#### Scenario: Respond to Identity Queries - successful outcome
...
- **WHEN** The actor needs to perform the 'Respond to Identity Queries' activity, or an authorised business process initiates the activity.
- **THEN** Participant / Patient initiates the 'Respond to Identity Queries' activity.
...
- **THEN** The 'Respond to Identity Queries' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
