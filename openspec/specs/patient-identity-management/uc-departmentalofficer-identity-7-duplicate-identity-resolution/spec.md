...
a single authoritative patient identity is maintained

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-Identity-7`
- **Source Scope:** Scope - Patient / Related Person Identity
- **Primary Actor:** Data Steward (Master Data)
- **Supporting Actors:** Provider Directory
- **User Story:** Merge confirmed duplicate patient records so that a single authoritative patient identity is maintained.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Duplicate Identity Resolution
Harmonia SHALL support **Duplicate Identity Resolution** so that a single authoritative patient identity is maintained.

#### Scenario: Duplicate Identity Resolution - successful outcome
...
- **WHEN** The actor needs to perform the 'Duplicate Identity Resolution' activity, or an authorised business process initiates the activity.
- **THEN** Data Steward (Master Data) initiates the 'Duplicate Identity Resolution' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Data Steward (Master Data) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Duplicate Identity Resolution' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Data Steward (Master Data)
