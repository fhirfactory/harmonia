# Track the Source of Each Provider Attribute

## Purpose

Enable the Provider Directory to retain attribute-level provenance whenever provider information is created or updated. This supports the target-state objective that sr-13 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that enriched records are auditable.

## Source Traceability

- **Source Use Case:** `SR-13`
...
- **User Story:** As a Compliance Officer, I want to track the source of data for each attribute, so that enriched records are auditable.
...
### Requirement: Track the Source of Each Provider Attribute
Harmonia SHALL support **Track the Source of Each Provider Attribute** so that Enable the Provider Directory to retain attribute-level provenance whenever provider information is created or updated. This supports the target-state objective that sr-13 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that enriched records are auditable.

#### Scenario: Track the Source of Each Provider Attribute - successful outcome
...
- **WHEN** The Compliance Officer initiates the 'Track the Source of Each Provider Attribute' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Compliance Officer initiates or causes the 'Track the Source of Each Provider Attribute' activity.
...
- **THEN** 6. The Provider Directory performs the required action to retain attribute-level provenance whenever provider information is created or updated.
...
- **THEN** The 'Track the Source of Each Provider Attribute' activity is completed successfully and retain attribute-level provenance whenever provider information is created or updated, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
