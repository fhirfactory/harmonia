# Associate Reference Data with Categories and Attributes

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-57 so that they can be used in the correct context. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-57`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Data Steward; Provider Directory
- **User Story:** As a Provider Directory Administrator, I want to associate reference data with categories of data or specific attributes so that they can be used in the correct context.
...
### Requirement: Associate Reference Data with Categories and Attr
Harmonia SHALL support **Associate Reference Data with Categories and Attributes** so that Enable the target-state Provider Directory to support the capability described by SR-57 so that they can be used in the correct context. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Associate Reference Data with Categories and Attributes - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-57.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Associate Reference Data with Categories and Attributes”.
...
- **THEN** The requirement described by SR-57 is completed successfully so that they can be used in the correct context. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Data Steward; Provider Directory
...
- **WHEN** Reference data can be assigned to categories such as specialties, roles and locations.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Relationships between reference data and entity types can be defined.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Dependent reference data can be defined, such as a specialty that applies only to a particular clinician type.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Reference data can be assigned to categories such as specialties, roles and locations.
...
- **GIVEN** Relationships between reference data and entity types can be defined.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Dependent reference data can be defined, such as a specialty that applies only to a particular clinician type.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
