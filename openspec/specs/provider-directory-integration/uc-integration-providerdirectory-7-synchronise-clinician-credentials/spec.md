# Synchronise Clinician Credentials

## Purpose

provider credentials remain current

## Source Traceability

- **Source Use Case:** `UC-Integration-ProviderDirectory-7`
...
- **User Story:** Exchange practitioner qualifications, registrations and certifications so that provider credentials remain current.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Synchronise Clinician Credentials
Harmonia SHALL support **Synchronise Clinician Credentials** so that provider credentials remain current.

#### Scenario: Synchronise Clinician Credentials - successful outcome
...
- **WHEN** The actor needs to perform the 'Synchronise Clinician Credentials' activity, or an authorised business process initiates the activity.
- **THEN** Credential / Workforce Management System initiates the 'Synchronise Clinician Credentials' activity.
...
- **THEN** The 'Synchronise Clinician Credentials' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
