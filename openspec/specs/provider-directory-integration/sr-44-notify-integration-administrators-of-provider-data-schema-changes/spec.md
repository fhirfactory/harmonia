...
- **User Story:** As a Provider Directory Administrator, I want to notify Integration administrators of changes to the provider data schema, so that external API versions can be updated.
...
### Requirement: Notify Integration Administrators of Provider Dat
Harmonia SHALL support **Notify Integration Administrators of Provider Data Schema Changes** so that Enable the target-state Provider Directory to support the capability described by SR-44 so that external API versions can be updated. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Notify Integration Administrators of Provider Data Schema Changes - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-44.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Notify Integration Administrators of Provider Data Schema Changes”.
...
- **THEN** The requirement described by SR-44 is completed successfully so that external API versions can be updated. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **WHEN** Version history is maintained for the provider data schema.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Schema changes can be communicated to API administrators.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Schema changes can be viewed and reviewed.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Version history is maintained for the provider data schema.
...
- **GIVEN** Schema changes can be communicated to API administrators.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Schema changes can be viewed and reviewed.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
