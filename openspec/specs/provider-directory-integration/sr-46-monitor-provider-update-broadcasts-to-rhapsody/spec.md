# Monitor Provider Update Broadcasts to Rhapsody

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-46 so that I can verify that updates are being distributed. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-46`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Integration Administrator; Rhapsody Integration Engine; Provider Directory; Integration Service
- **User Story:** As a Provider Directory Administrator, I want to monitor broadcast activity to Rhapsody, so that I can verify that updates are being distributed.
...
### Requirement: Monitor Provider Update Broadcasts to Rhapsody
Harmonia SHALL support **Monitor Provider Update Broadcasts to Rhapsody** so that Enable the target-state Provider Directory to support the capability described by SR-46 so that I can verify that updates are being distributed. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Monitor Provider Update Broadcasts to Rhapsody - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-46.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Monitor Provider Update Broadcasts to Rhapsody”.
...
- **THEN** The requirement described by SR-46 is completed successfully so that I can verify that updates are being distributed. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Integration Administrator; Rhapsody Integration Engine; Provider Directory; Integration Service
...
- **WHEN** Recent HL7/FHIR message activity can be monitored.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Delayed outbound messages can generate a notification.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Failed outbound messages can generate a notification.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Messages can be resent manually where authorised.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Recent HL7/FHIR message activity can be monitored.
...
- **GIVEN** Delayed outbound messages can generate a notification.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Failed outbound messages can generate a notification.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Messages can be resent manually where authorised.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
