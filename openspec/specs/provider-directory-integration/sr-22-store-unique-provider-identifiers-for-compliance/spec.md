# Store Unique Provider Identifiers for Compliance

## Purpose

Enable the Provider Directory to store, validate and protect recognised identifiers such as HPI-I, HPI-O or AHPRA registration. This supports the target-state objective that sr-22 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can meet ADHA compliance requirements.

## Source Traceability

- **Source Use Case:** `SR-22`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Compliance Officer
- **Supporting Actors:** Provider Directory Administrator; Provider Directory; Integration Service; Provider Identity / Matching Service
- **User Story:** As a Compliance Officer, I want to store a unique provider identifier, so that I can meet ADHA compliance requirements.
...
### Requirement: Store Unique Provider Identifiers for Compliance
Harmonia SHALL support **Store Unique Provider Identifiers for Compliance** so that Enable the Provider Directory to store, validate and protect recognised identifiers such as HPI-I, HPI-O or AHPRA registration. This supports the target-state objective that sr-22 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can meet ADHA compliance requirements.

#### Scenario: Store Unique Provider Identifiers for Compliance - successful outcome
...
- **WHEN** The Compliance Officer initiates the 'Store Unique Provider Identifiers for Compliance' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Compliance Officer initiates or causes the 'Store Unique Provider Identifiers for Compliance' activity.
...
- **THEN** 6. The Provider Directory performs the required action to store, validate and protect recognised identifiers such as HPI-I, HPI-O or AHPRA registration.
...
- **THEN** The 'Store Unique Provider Identifiers for Compliance' activity is completed successfully and store, validate and protect recognised identifiers such as HPI-I, HPI-O or AHPRA registration, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Provider Directory Administrator; Provider Directory; Integration Service; Provider Identity / Matching Service
