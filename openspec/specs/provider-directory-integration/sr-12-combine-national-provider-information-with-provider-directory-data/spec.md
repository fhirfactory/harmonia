# Combine National Provider Information with Provider Directory Data

## Purpose

Enable the Provider Directory to apply information from multiple recognised sources to one provider record. This supports the target-state objective that sr-12 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that a single, complete record is maintained.

## Source Traceability

- **Source Use Case:** `SR-12`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Provider Identity / Matching Service; Provider Directory
- **User Story:** As an Integration Administrator, I want to combine national provider information with Provider Directory data, so that a single, complete record is maintained.
...
### Requirement: Combine National Provider Information with Provid
Harmonia SHALL support **Combine National Provider Information with Provider Directory Data** so that Enable the Provider Directory to apply information from multiple recognised sources to one provider record. This supports the target-state objective that sr-12 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that a single, complete record is maintained.

#### Scenario: Combine National Provider Information with Provider Directory Data - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Combine National Provider Information with Provider Directory Data' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Combine National Provider Information with Provider Directory Data' activity.
...
- **THEN** 6. The Provider Directory performs the required action to apply information from multiple recognised sources to one provider record.
...
- **THEN** The 'Combine National Provider Information with Provider Directory Data' activity is completed successfully and apply information from multiple recognised sources to one provider record, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Provider Identity / Matching Service; Provider Directory
