# Receive Provider Details via Integration

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-33 so that system users can reliably access provider details. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-33`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** System Administrator
- **Supporting Actors:** Provider Directory; Integration Service
- **User Story:** As a System Administrator, I want to receive provider details via integration, so that system users can reliably access provider details.
...
### Requirement: Receive Provider Details via Integration
Harmonia SHALL support **Receive Provider Details via Integration** so that Enable the target-state Provider Directory to support the capability described by SR-33 so that system users can reliably access provider details. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Receive Provider Details via Integration - successful outcome
...
- **WHEN** The System Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-33.
- **THEN** 1. The System Administrator initiates or receives the event associated with “Receive Provider Details via Integration”.
...
- **THEN** The requirement described by SR-33 is completed successfully so that system users can reliably access provider details. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Provider Directory; Integration Service
...
- **WHEN** The receiving system obtains provider data using the expected data structure.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** The supplied information is sufficiently detailed for downstream system needs.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Provider status is included where required by the downstream system.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Changes are supplied within an acceptable timeframe.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** The receiving system obtains provider data using the expected data structure.
...
- **GIVEN** The supplied information is sufficiently detailed for downstream system needs.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Provider status is included where required by the downstream system.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Changes are supplied within an acceptable timeframe.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
