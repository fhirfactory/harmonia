# Receive Provider Information from National Data Sources

## Purpose

Enable the Provider Directory to receive provider information from multiple national sources through standard interfaces and retain source and error information. This supports the target-state objective that sr-7 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that provider information is updated from authoritative systems.

## Source Traceability

- **Source Use Case:** `SR-7`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** National Provider Data Source; Provider Directory; Provider Identity / Matching Service; Data Steward
- **User Story:** As an Integration Administrator, I want the Provider Directory to receive provider information from national data sources (such as NHSD, ADHA, or Medicare), so that provider information is updated from authoritative systems.
...
### Requirement: Receive Provider Information from National Data S
Harmonia SHALL support **Receive Provider Information from National Data Sources** so that Enable the Provider Directory to receive provider information from multiple national sources through standard interfaces and retain source and error information. This supports the target-state objective that sr-7 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that provider information is updated from authoritative systems.

#### Scenario: Receive Provider Information from National Data Sources - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Receive Provider Information from National Data Sources' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Receive Provider Information from National Data Sources' activity.
...
- **THEN** 6. The Provider Directory performs the required action to receive provider information from multiple national sources through standard interfaces and retain source and error information.
...
- **THEN** The 'Receive Provider Information from National Data Sources' activity is completed successfully and receive provider information from multiple national sources through standard interfaces and retain source and error information, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by National Provider Data Source; Provider Directory; Provider Identity / Matching Service; Data Steward
