...
- **User Story:** As a Provider Directory Administrator, I want to create, update, and deactivate reference data sets, so that the directory supports standardised data entry.
...
### Requirement: Create Update and Deactivate Reference Data Sets
Harmonia SHALL support **Create Update and Deactivate Reference Data Sets** so that Enable the target-state Provider Directory to support the capability described by SR-52 so that the directory supports standardised data entry. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Create Update and Deactivate Reference Data Sets - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-52.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Create Update and Deactivate Reference Data Sets”.
...
- **THEN** The requirement described by SR-52 is completed successfully so that the directory supports standardised data entry. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **WHEN** Reference data sets can be created, updated and deactivated through a user interface.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Reference data sets can be versioned.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Reference data sets can be created, updated and deactivated through a user interface.
...
- **GIVEN** Reference data sets can be versioned.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
