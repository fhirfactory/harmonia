# Ensure Attributes Are Updated Only by Approved Sources

## Purpose

Enable the Provider Directory to enforce authoritative-source controls and retain complete attribute-level change history, source, method and timestamp. This supports the target-state objective that sr-16 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that data integrity and policy requirements are met.

## Source Traceability

- **Source Use Case:** `SR-16`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Compliance Officer
- **Supporting Actors:** Provider Directory Administrator; Provider Directory; Integration Service; Data Steward
- **User Story:** As a Compliance Officer, I want to ensure that each provider attribute is updated only by an approved source, so that data integrity and policy requirements are met.
...
### Requirement: Ensure Attributes Are Updated Only by Approved So
Harmonia SHALL support **Ensure Attributes Are Updated Only by Approved Sources** so that Enable the Provider Directory to enforce authoritative-source controls and retain complete attribute-level change history, source, method and timestamp. This supports the target-state objective that sr-16 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that data integrity and policy requirements are met.

#### Scenario: Ensure Attributes Are Updated Only by Approved Sources - successful outcome
...
- **WHEN** The Compliance Officer initiates the 'Ensure Attributes Are Updated Only by Approved Sources' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Compliance Officer initiates or causes the 'Ensure Attributes Are Updated Only by Approved Sources' activity.
...
- **THEN** 6. The Provider Directory performs the required action to enforce authoritative-source controls and retain complete attribute-level change history, source, method and timestamp.
...
- **THEN** The 'Ensure Attributes Are Updated Only by Approved Sources' activity is completed successfully and enforce authoritative-source controls and retain complete attribute-level change history, source, method and timestamp, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Provider Directory Administrator; Provider Directory; Integration Service; Data Steward
