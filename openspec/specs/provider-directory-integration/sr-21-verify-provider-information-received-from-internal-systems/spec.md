...
### Requirement: Verify Provider Information Received from Interna
Harmonia SHALL support **Verify Provider Information Received from Internal Systems** so that Enable the Provider Directory to validate and transform incoming internal provider information before acceptance. This supports the target-state objective that sr-21 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incorrect records are not stored.

#### Scenario: Verify Provider Information Received from Internal Systems - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Verify Provider Information Received from Internal Systems' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Verify Provider Information Received from Internal Systems' activity.
...
- **THEN** 6. The Provider Directory performs the required action to validate and transform incoming internal provider information before acceptance.
...
- **THEN** The 'Verify Provider Information Received from Internal Systems' activity is completed successfully and validate and transform incoming internal provider information before acceptance, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
