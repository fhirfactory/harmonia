# Validate HPI-I or HPI-O for Compliance

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-30 so that we meet national compliance requirements. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-30`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Compliance Officer
- **Supporting Actors:** Provider Directory Administrator; Provider Identity / Matching Service; Provider Directory
- **User Story:** As a Compliance Officer, I want to ensure that each provider record includes a validated HPI-I or HPI-O, so that we meet national compliance requirements.
...
### Requirement: Validate HPI-I or HPI-O for Compliance
Harmonia SHALL support **Validate HPI-I or HPI-O for Compliance** so that Enable the target-state Provider Directory to support the capability described by SR-30 so that we meet national compliance requirements. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Validate HPI-I or HPI-O for Compliance - successful outcome
...
- **WHEN** The Compliance Officer initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-30.
- **THEN** 1. The Compliance Officer initiates or receives the event associated with “Validate HPI-I or HPI-O for Compliance”.
...
- **THEN** The requirement described by SR-30 is completed successfully so that we meet national compliance requirements. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Provider Directory Administrator; Provider Identity / Matching Service; Provider Directory
...
- **WHEN** The HPI is verified when a provider record is created or updated.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Invalid or potentially duplicate HPIs are rejected or marked for follow-up.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** A report can be generated showing identity validation status.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** The HPI is verified when a provider record is created or updated.
...
- **GIVEN** Invalid or potentially duplicate HPIs are rejected or marked for follow-up.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** A report can be generated showing identity validation status.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
