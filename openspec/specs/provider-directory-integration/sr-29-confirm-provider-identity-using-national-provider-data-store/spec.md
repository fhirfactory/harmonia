# Confirm Provider Identity Using National Provider Data Store

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-29 so that the correct provider details are stored in the directory. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-29`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Services Australia / HI Service; Provider Identity / Matching Service; Provider Directory
- **User Story:** As an Integration Administrator, I want to confirm a provider’s identity using the Services Australia / HI Service, so that the correct provider details are stored in the directory.
- **Related Use Cases:** UC-Integration-ProviderDirectory-30 – Match Provider Records: supports provider identity resolution and duplicate avoidance.; UC-Integration-ProviderDirectory-31 – Receive Provider Identifier Cross References: supports reconciliation of identifiers across source systems.

## Requirements

### Requirement: Confirm Provider Identity Using National Provider
Harmonia SHALL support **Confirm Provider Identity Using National Provider Data Store** so that Enable the target-state Provider Directory to support the capability described by SR-29 so that the correct provider details are stored in the directory. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Confirm Provider Identity Using National Provider Data Store - successful outcome
...
- **WHEN** The Integration Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-29.
- **THEN** 1. The Integration Administrator initiates or receives the event associated with “Confirm Provider Identity Using National Provider Data Store”.
...
- **THEN** The requirement described by SR-29 is completed successfully so that the correct provider details are stored in the directory. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Services Australia / HI Service; Provider Identity / Matching Service; Provider Directory
...
- **WHEN** Provider identity can be checked using HPI-I or HPI-O against the National Provider Data Store.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Demographic and location attributes can be used to support identity verification.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Ambiguous results are flagged for follow-up.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** The identity verification outcome is recorded.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **WHEN** UC-Integration-ProviderDirectory-30 – Match Provider Records: supports provider identity resolution and duplicate avoidance.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 19
- **WHEN** UC-Integration-ProviderDirectory-31 – Receive Provider Identifier Cross References: supports reconciliation of identifiers across source systems.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Provider identity can be checked using HPI-I or HPI-O against the National Provider Data Store.
...
- **GIVEN** Demographic and location attributes can be used to support identity verification.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Ambiguous results are flagged for follow-up.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** The identity verification outcome is recorded.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
