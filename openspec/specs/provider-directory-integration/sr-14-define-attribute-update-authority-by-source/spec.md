# Define Attribute Update Authority by Source

## Purpose

Enable the Provider Directory to configure and enforce source-of-truth rules by provider attribute and source system. This supports the target-state objective that sr-14 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the Provider Directory only accepts trusted data.

## Source Traceability

- **Source Use Case:** `SR-14`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Provider Directory Administrator; Provider Directory; Data Steward
- **User Story:** As an Integration Administrator, I want to define which attribute can be updated from specific sources, so that the Provider Directory only accepts trusted data.
...
### Requirement: Define Attribute Update Authority by Source
Harmonia SHALL support **Define Attribute Update Authority by Source** so that Enable the Provider Directory to configure and enforce source-of-truth rules by provider attribute and source system. This supports the target-state objective that sr-14 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the Provider Directory only accepts trusted data.

#### Scenario: Define Attribute Update Authority by Source - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Define Attribute Update Authority by Source' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Define Attribute Update Authority by Source' activity.
...
- **THEN** 6. The Provider Directory performs the required action to configure and enforce source-of-truth rules by provider attribute and source system.
...
- **THEN** The 'Define Attribute Update Authority by Source' activity is completed successfully and configure and enforce source-of-truth rules by provider attribute and source system, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Provider Directory Administrator; Provider Directory; Data Steward
