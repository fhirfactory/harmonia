# Match Internal Provider Data Using Unique Identifiers

## Purpose

Enable the Provider Directory to use unique identifiers to find existing providers before creation and flag ambiguous matches. This supports the target-state objective that sr-23 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that duplicate record creation is avoided.

## Source Traceability

- **Source Use Case:** `SR-23`
...
### Requirement: Match Internal Provider Data Using Unique Identif
Harmonia SHALL support **Match Internal Provider Data Using Unique Identifiers** so that Enable the Provider Directory to use unique identifiers to find existing providers before creation and flag ambiguous matches. This supports the target-state objective that sr-23 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that duplicate record creation is avoided.

#### Scenario: Match Internal Provider Data Using Unique Identifiers - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Match Internal Provider Data Using Unique Identifiers' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Match Internal Provider Data Using Unique Identifiers' activity.
...
- **THEN** 6. The Provider Directory performs the required action to use unique identifiers to find existing providers before creation and flag ambiguous matches.
