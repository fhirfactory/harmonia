# Receive Care Provider Details from National Provider Data Store

## Purpose

Enable the Provider Directory to receive provider information from national provider data sources. This supports the target-state objective that uc-2 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that provider information maintained by national authoritative or reference sources is incorporated into the Provider Directory.

## Source Traceability

- **Source Use Case:** `UC-2`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** National Provider Data Store
- **Supporting Actors:** Integration Administrator; Provider Directory; Provider Identity / Matching Service
- **User Story:** As the Provider Directory, I need to receive provider information from national provider data sources, so that provider information maintained by national authoritative or reference sources is incorporated into the Provider Directory.
...
### Requirement: Receive Care Provider Details from National Provi
Harmonia SHALL support **Receive Care Provider Details from National Provider Data Store** so that Enable the Provider Directory to receive provider information from national provider data sources. This supports the target-state objective that uc-2 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that provider information maintained by national authoritative or reference sources is incorporated into the Provider Directory.

#### Scenario: Receive Care Provider Details from National Provider Data Store - successful outcome
...
- **WHEN** The National Provider Data Store initiates the 'Receive Care Provider Details from National Provider Data Store' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The National Provider Data Store initiates or causes the 'Receive Care Provider Details from National Provider Data Store' activity.
...
- **THEN** 6. The Provider Directory performs the required action to receive provider information from national provider data sources.
...
- **THEN** The 'Receive Care Provider Details from National Provider Data Store' activity is completed successfully and receive provider information from national provider data sources, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **GIVEN** the primary actor is National Provider Data Store
- **AND** supporting participation may be provided by Integration Administrator; Provider Directory; Provider Identity / Matching Service
