# Alert Administrators When Provider Registration Status Changes

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-28 so that I can ensure all systems are aware of the changes. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-28`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Compliance Officer; Integration Administrator; Provider Directory
- **User Story:** As a Provider Directory Administrator, I want to be alerted when a provider’s status changes, so that I can ensure all systems are aware of the changes.
...
### Requirement: Alert Administrators When Provider Registration S
Harmonia SHALL support **Alert Administrators When Provider Registration Status Changes** so that Enable the target-state Provider Directory to support the capability described by SR-28 so that I can ensure all systems are aware of the changes. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Alert Administrators When Provider Registration Status Changes - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-28.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Alert Administrators When Provider Registration Status Changes”.
...
- **THEN** The requirement described by SR-28 is completed successfully so that I can ensure all systems are aware of the changes. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Compliance Officer; Integration Administrator; Provider Directory
...
- **WHEN** Provider status changes are flagged.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Configured users can receive notifications about status changes.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** A report can be generated showing provider status changes.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Provider status changes are flagged.
...
- **GIVEN** Configured users can receive notifications about status changes.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** A report can be generated showing provider status changes.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
