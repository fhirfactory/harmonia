# Verify Provider Registration Status Against National Provider Data Store

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-26 so that only currently registered clinicians are active in the provider directory. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-26`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** National Provider Data Store; Compliance Officer; Provider Directory
- **User Story:** As an Integration Administrator, I want to verify a provider’s registration status using the National Provider Data Store, so that only currently registered clinicians are active in the provider directory.
- **Related Use Cases:** UC-Integration-ProviderDirectory-37 – Receive External Provider Details from Provider Data Store: provides an external provider information source and associated provenance controls.

## Requirements

### Requirement: Verify Provider Registration Status Against Natio
Harmonia SHALL support **Verify Provider Registration Status Against National Provider Data Store** so that Enable the target-state Provider Directory to support the capability described by SR-26 so that only currently registered clinicians are active in the provider directory. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Verify Provider Registration Status Against National Provider Data Store - successful outcome
...
- **WHEN** The Integration Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-26.
- **THEN** 1. The Integration Administrator initiates or receives the event associated with “Verify Provider Registration Status Against National Provider Data Store”.
...
- **THEN** The requirement described by SR-26 is completed successfully so that only currently registered clinicians are active in the provider directory. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by National Provider Data Store; Compliance Officer; Provider Directory
...
- **WHEN** Provider status can be verified against the National Provider Data Store (AHPRA).
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** The provider status field is updated when the authoritative status changes.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** The status received is validated against recognised values such as active, suspended, expired or cancelled.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Verification can run on a regular schedule or on demand.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 13
- **WHEN** Invalid or unrecognised statuses are logged for review.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **WHEN** UC-Integration-ProviderDirectory-37 – Receive External Provider Details from Provider Data Store: provides an external provider information source and associated provenance controls.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Provider status can be verified against the National Provider Data Store (AHPRA).
...
- **GIVEN** The provider status field is updated when the authoritative status changes.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** The status received is validated against recognised values such as active, suspended, expired or cancelled.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Verification can run on a regular schedule or on demand.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 5
- **GIVEN** Invalid or unrecognised statuses are logged for review.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
