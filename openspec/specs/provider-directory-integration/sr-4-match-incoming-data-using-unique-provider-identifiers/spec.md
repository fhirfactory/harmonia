# Match Incoming Data Using Unique Provider Identifiers

## Purpose

Enable the Provider Directory to match incoming records to existing providers before creating new records and flag ambiguous matches. This supports the target-state objective that sr-4 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that duplicate record creation is avoided.

## Source Traceability

- **Source Use Case:** `SR-4`
...
- **User Story:** As an Integration Administrator, I want to use unique provider identifiers to match with existing records, so that duplicate record creation is avoided.
...
### Requirement: Match Incoming Data Using Unique Provider Identif
Harmonia SHALL support **Match Incoming Data Using Unique Provider Identifiers** so that Enable the Provider Directory to match incoming records to existing providers before creating new records and flag ambiguous matches. This supports the target-state objective that sr-4 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that duplicate record creation is avoided.

#### Scenario: Match Incoming Data Using Unique Provider Identifiers - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Match Incoming Data Using Unique Provider Identifiers' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Match Incoming Data Using Unique Provider Identifiers' activity.
...
- **THEN** 6. The Provider Directory performs the required action to match incoming records to existing providers before creating new records and flag ambiguous matches.
...
- **THEN** The 'Match Incoming Data Using Unique Provider Identifiers' activity is completed successfully and match incoming records to existing providers before creating new records and flag ambiguous matches, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
