# Match Provider Records

## Purpose

duplicate provider profiles are avoided

## Source Traceability

- **Source Use Case:** `UC-Integration-ProviderDirectory-30`
...
- **User Story:** Match provider records received from external systems so that duplicate provider profiles are avoided.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Match Provider Records
Harmonia SHALL support **Match Provider Records** so that duplicate provider profiles are avoided.

#### Scenario: Match Provider Records - successful outcome
...
- **WHEN** The actor needs to perform the 'Match Provider Records' activity, or an authorised business process initiates the activity.
- **THEN** Downstream Consumer System System initiates the 'Match Provider Records' activity.
...
- **THEN** The 'Match Provider Records' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
