# Broadcast Provider Updates to Rhapsody Using Standard Messaging

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-45 so that subscribing systems receive timely updates. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-45`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Rhapsody Integration Engine; Downstream Subscriber System; Provider Directory; Integration Service
- **User Story:** As an Integration Administrator, I want the Provider Directory to broadcast provider updates to Rhapsody using standard messaging, so that subscribing systems receive timely updates.
...
### Requirement: Broadcast Provider Updates to Rhapsody Using Stan
Harmonia SHALL support **Broadcast Provider Updates to Rhapsody Using Standard Messaging** so that Enable the target-state Provider Directory to support the capability described by SR-45 so that subscribing systems receive timely updates. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Broadcast Provider Updates to Rhapsody Using Standard Messaging - successful outcome
...
- **WHEN** The Integration Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-45.
- **THEN** 1. The Integration Administrator initiates or receives the event associated with “Broadcast Provider Updates to Rhapsody Using Standard Messaging”.
...
- **THEN** The requirement described by SR-45 is completed successfully so that subscribing systems receive timely updates. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Rhapsody Integration Engine; Downstream Subscriber System; Provider Directory; Integration Service
...
- **WHEN** HL7 messages can be generated for provider updates using a defined schema, including MFN where applicable.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** FHIR messages can be generated for provider updates using a defined schema.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Messages use the common messaging model where applicable.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Messages can be sent to Rhapsody.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 13
- **WHEN** Messages can be sent in real time or on a schedule.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 14
- **WHEN** Message-send failures are logged.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** HL7 messages can be generated for provider updates using a defined schema, including MFN where applicable.
...
- **GIVEN** FHIR messages can be generated for provider updates using a defined schema.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Messages use the common messaging model where applicable.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Messages can be sent to Rhapsody.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 5
- **GIVEN** Messages can be sent in real time or on a schedule.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 6
- **GIVEN** Message-send failures are logged.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
