# Verify Provider Information Received from National Sources

## Purpose

Enable the Provider Directory to validate and transform incoming national provider information before it is accepted. This supports the target-state objective that sr-8 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incorrect records are not stored.

## Source Traceability

- **Source Use Case:** `SR-8`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Data Steward; Provider Directory; Provider Identity / Matching Service; Data Steward
...
### Requirement: Verify Provider Information Received from Nationa
Harmonia SHALL support **Verify Provider Information Received from National Sources** so that Enable the Provider Directory to validate and transform incoming national provider information before it is accepted. This supports the target-state objective that sr-8 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incorrect records are not stored.

#### Scenario: Verify Provider Information Received from National Sources - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Verify Provider Information Received from National Sources' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Verify Provider Information Received from National Sources' activity.
...
- **THEN** 6. The Provider Directory performs the required action to validate and transform incoming national provider information before it is accepted.
...
- **THEN** The 'Verify Provider Information Received from National Sources' activity is completed successfully and validate and transform incoming national provider information before it is accepted, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Data Steward; Provider Directory; Provider Identity / Matching Service; Data Steward
