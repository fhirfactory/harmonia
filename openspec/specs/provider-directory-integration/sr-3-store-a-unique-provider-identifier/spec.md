# Store a Unique Provider Identifier

## Purpose

Enable the Provider Directory to store and protect a verified unique provider identifier such as HPI-I, HPI-O or XCode. This supports the target-state objective that sr-3 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the correct record can be matched to incoming data.

## Source Traceability

- **Source Use Case:** `SR-3`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Compliance Officer; Provider Directory; Integration Service; Provider Identity / Matching Service
- **User Story:** As a Provider Directory Administrator, I want to store a unique provider identifier, so that the correct record can be matched to incoming data.
...
### Requirement: Store a Unique Provider Identifier
Harmonia SHALL support **Store a Unique Provider Identifier** so that Enable the Provider Directory to store and protect a verified unique provider identifier such as HPI-I, HPI-O or XCode. This supports the target-state objective that sr-3 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the correct record can be matched to incoming data.

#### Scenario: Store a Unique Provider Identifier - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the 'Store a Unique Provider Identifier' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Provider Directory Administrator initiates or causes the 'Store a Unique Provider Identifier' activity.
...
- **THEN** 6. The Provider Directory performs the required action to store and protect a verified unique provider identifier such as HPI-I, HPI-O or XCode.
...
- **THEN** The 'Store a Unique Provider Identifier' activity is completed successfully and store and protect a verified unique provider identifier such as HPI-I, HPI-O or XCode, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Compliance Officer; Provider Directory; Integration Service; Provider Identity / Matching Service
