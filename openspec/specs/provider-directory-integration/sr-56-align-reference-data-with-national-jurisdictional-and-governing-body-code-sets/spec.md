# Align Reference Data with National Jurisdictional and Governing Body Code Sets

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-56 so that interoperability is supported. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-56`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Compliance Officer
- **Supporting Actors:** Provider Directory Administrator; Integration Administrator; Provider Directory
- **User Story:** As a Compliance Officer, I want use reference data sets that are aligned with national, jurisdictional and governing body code sets, so that interoperability is supported.
...
### Requirement: Align Reference Data with National Jurisdictional
Harmonia SHALL support **Align Reference Data with National Jurisdictional and Governing Body Code Sets** so that Enable the target-state Provider Directory to support the capability described by SR-56 so that interoperability is supported. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Align Reference Data with National Jurisdictional and Governing Body Code Sets - successful outcome
...
- **WHEN** The Compliance Officer initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-56.
- **THEN** 1. The Compliance Officer initiates or receives the event associated with “Align Reference Data with National Jurisdictional and Governing Body Code Sets”.
...
- **THEN** The requirement described by SR-56 is completed successfully so that interoperability is supported. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Provider Directory Administrator; Integration Administrator; Provider Directory
...
- **WHEN** Reference data sets can be imported from external sources using recognised formats such as API, XML or CSV.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** External code sets such as AHPRA or NHSD can be imported where available and authorised.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Internal code sets such as HR, TRDF or DoH reference data can be imported where required.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Reference data sets can be imported from external sources using recognised formats such as API, XML or CSV.
...
- **GIVEN** External code sets such as AHPRA or NHSD can be imported where available and authorised.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Internal code sets such as HR, TRDF or DoH reference data can be imported where required.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
