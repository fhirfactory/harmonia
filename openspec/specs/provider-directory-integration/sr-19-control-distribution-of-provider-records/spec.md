# Control Distribution of Provider Records

## Purpose

Enable the Provider Directory to restrict or approve distribution based on provider status and governance rules. This supports the target-state objective that sr-19 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incomplete records are not shared.

## Source Traceability

- **Source Use Case:** `SR-19`
...
- **User Story:** As a Provider Directory Administrator, I want to control when provider records are distributed to downstream systems, so that incomplete records are not shared.
...
### Requirement: Control Distribution of Provider Records
Harmonia SHALL support **Control Distribution of Provider Records** so that Enable the Provider Directory to restrict or approve distribution based on provider status and governance rules. This supports the target-state objective that sr-19 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incomplete records are not shared.

#### Scenario: Control Distribution of Provider Records - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the 'Control Distribution of Provider Records' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Provider Directory Administrator initiates or causes the 'Control Distribution of Provider Records' activity.
...
- **THEN** 6. The Provider Directory performs the required action to restrict or approve distribution based on provider status and governance rules.
