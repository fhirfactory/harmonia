# View and Maintain Attribute Source Update Rules

## Purpose

Enable the Provider Directory to view, search, filter, maintain and export source update rules, including conditional rules. This supports the target-state objective that sr-17 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can understand and manage the source of truth for each attribute.

## Source Traceability

- **Source Use Case:** `SR-17`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Integration Administrator; Provider Directory; Data Steward
- **User Story:** As a Provider Directory Administrator, I want to view update rules by attribute and source system, so that I can understand and manage the source of truth for each attribute.
...
### Requirement: View and Maintain Attribute Source Update Rules
Harmonia SHALL support **View and Maintain Attribute Source Update Rules** so that Enable the Provider Directory to view, search, filter, maintain and export source update rules, including conditional rules. This supports the target-state objective that sr-17 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can understand and manage the source of truth for each attribute.

#### Scenario: View and Maintain Attribute Source Update Rules - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the 'View and Maintain Attribute Source Update Rules' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Provider Directory Administrator initiates or causes the 'View and Maintain Attribute Source Update Rules' activity.
...
- **THEN** 6. The Provider Directory performs the required action to view, search, filter, maintain and export source update rules, including conditional rules.
...
- **THEN** The 'View and Maintain Attribute Source Update Rules' activity is completed successfully and view, search, filter, maintain and export source update rules, including conditional rules, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Integration Administrator; Provider Directory; Data Steward
