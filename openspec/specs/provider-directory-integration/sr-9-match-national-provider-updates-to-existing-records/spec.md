# Match National Provider Updates to Existing Records

## Purpose

Enable the Provider Directory to use unique identifiers to update the correct existing provider and log ambiguous matches. This supports the target-state objective that sr-9 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the correct record is updated.

## Source Traceability

- **Source Use Case:** `SR-9`
...
- **User Story:** As an Integration Administrator, I want to use unique provider identifiers to match with existing records, so that the correct record is updated.
...
### Requirement: Match National Provider Updates to Existing Recor
Harmonia SHALL support **Match National Provider Updates to Existing Records** so that Enable the Provider Directory to use unique identifiers to update the correct existing provider and log ambiguous matches. This supports the target-state objective that sr-9 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the correct record is updated.

#### Scenario: Match National Provider Updates to Existing Records - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Match National Provider Updates to Existing Records' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Match National Provider Updates to Existing Records' activity.
...
- **THEN** 6. The Provider Directory performs the required action to use unique identifiers to update the correct existing provider and log ambiguous matches.
...
- **THEN** The 'Match National Provider Updates to Existing Records' activity is completed successfully and use unique identifiers to update the correct existing provider and log ambiguous matches, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
