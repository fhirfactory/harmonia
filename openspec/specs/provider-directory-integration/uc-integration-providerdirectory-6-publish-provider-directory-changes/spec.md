# Publish Provider Directory Changes

## Purpose

subscribed systems can synchronise provider information

## Source Traceability

- **Source Use Case:** `UC-Integration-ProviderDirectory-6`
- **Source Scope:** Scope - Provider Profile Synchronisation
- **Primary Actor:** Provider Directory
- **Supporting Actors:** Provider Directory
- **User Story:** Publish provider changes so that subscribed systems can synchronise provider information.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Publish Provider Directory Changes
Harmonia SHALL support **Publish Provider Directory Changes** so that subscribed systems can synchronise provider information.

#### Scenario: Publish Provider Directory Changes - successful outcome
...
- **WHEN** The actor needs to perform the 'Publish Provider Directory Changes' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory initiates the 'Publish Provider Directory Changes' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Provider Directory enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Publish Provider Directory Changes' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Provider Directory
