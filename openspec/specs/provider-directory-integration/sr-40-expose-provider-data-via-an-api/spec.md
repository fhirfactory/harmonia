# Expose Provider Data via an API

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-40 so that systems can access the data securely. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-40`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Downstream Consumer System; API Management Service; Provider Directory; Integration Service
- **User Story:** As an Integration Administrator, I want the Provider Directory to expose provider data via an API so that systems can access the data securely.
...
### Requirement: Expose Provider Data via an API
Harmonia SHALL support **Expose Provider Data via an API** so that Enable the target-state Provider Directory to support the capability described by SR-40 so that systems can access the data securely. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Expose Provider Data via an API - successful outcome
...
- **WHEN** The Integration Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-40.
- **THEN** 1. The Integration Administrator initiates or receives the event associated with “Expose Provider Data via an API”.
...
- **THEN** The requirement described by SR-40 is completed successfully so that systems can access the data securely. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Downstream Consumer System; API Management Service; Provider Directory; Integration Service
...
- **WHEN** Provider data is exposed through an API.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** The API is documented.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Connections between the Provider Directory and API consumers are identifiable and manageable.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Connections between the Provider Directory and API consumers are secure.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Provider data is exposed through an API.
...
- **GIVEN** Connections between the Provider Directory and API consumers are identifiable and manageable.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Connections between the Provider Directory and API consumers are secure.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
