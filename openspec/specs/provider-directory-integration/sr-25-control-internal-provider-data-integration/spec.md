# Control Internal Provider Data Integration

## Purpose

Enable the Provider Directory to schedule, trigger, pause and resume internal provider data feeds. This supports the target-state objective that sr-25 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can effectively manage system resources.

## Source Traceability

- **Source Use Case:** `SR-25`
...
- **User Story:** As an Integration Administrator, I want to control when provider information is added to the Provider Directory, so that I can effectively manage system resources.
...
### Requirement: Control Internal Provider Data Integration
Harmonia SHALL support **Control Internal Provider Data Integration** so that Enable the Provider Directory to schedule, trigger, pause and resume internal provider data feeds. This supports the target-state objective that sr-25 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can effectively manage system resources.

#### Scenario: Control Internal Provider Data Integration - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Control Internal Provider Data Integration' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Control Internal Provider Data Integration' activity.
...
- **THEN** 6. The Provider Directory performs the required action to schedule, trigger, pause and resume internal provider data feeds.
...
- **THEN** The 'Control Internal Provider Data Integration' activity is completed successfully and schedule, trigger, pause and resume internal provider data feeds, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
