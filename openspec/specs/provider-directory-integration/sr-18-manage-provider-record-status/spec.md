# Manage Provider Record Status

## Purpose

Enable the Provider Directory to manage provider lifecycle statuses and use them to govern processing and distribution. This supports the target-state objective that sr-18 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can manage the lifecycle of the record.

## Source Traceability

- **Source Use Case:** `SR-18`
...
- **User Story:** As a Provider Directory Administrator, I want provider records to have a status, so that I can manage the lifecycle of the record.
...
### Requirement: Manage Provider Record Status
Harmonia SHALL support **Manage Provider Record Status** so that Enable the Provider Directory to manage provider lifecycle statuses and use them to govern processing and distribution. This supports the target-state objective that sr-18 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can manage the lifecycle of the record.

#### Scenario: Manage Provider Record Status - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the 'Manage Provider Record Status' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Provider Directory Administrator initiates or causes the 'Manage Provider Record Status' activity.
...
- **THEN** 6. The Provider Directory performs the required action to manage provider lifecycle statuses and use them to govern processing and distribution.
...
- **THEN** The 'Manage Provider Record Status' activity is completed successfully and manage provider lifecycle statuses and use them to govern processing and distribution, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
