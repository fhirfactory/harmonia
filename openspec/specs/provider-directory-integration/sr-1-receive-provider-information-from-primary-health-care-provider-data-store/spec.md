# Receive Provider Information from Primary Health Care Provider Data Store

## Purpose

Enable the Provider Directory to receive external provider data through standard integration while retaining source and error information. This supports the target-state objective that sr-1 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the directory of external providers is kept up to date.

## Source Traceability

- **Source Use Case:** `SR-1`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Primary Health Care Provider Data Store; Provider Directory; Provider Identity / Matching Service
- **User Story:** As an Integration Administrator, I want the Provider Directory to receive provider information from the Primary Health Care Provider Data Store, so that the directory of external providers is kept up to date.
...
### Requirement: Receive Provider Information from Primary Health
Harmonia SHALL support **Receive Provider Information from Primary Health Care Provider Data Store** so that Enable the Provider Directory to receive external provider data through standard integration while retaining source and error information. This supports the target-state objective that sr-1 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the directory of external providers is kept up to date.

#### Scenario: Receive Provider Information from Primary Health Care Provider Data Store - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Receive Provider Information from Primary Health Care Provider Data Store' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Receive Provider Information from Primary Health Care Provider Data Store' activity.
...
- **THEN** 6. The Provider Directory performs the required action to receive external provider data through standard integration while retaining source and error information.
...
- **THEN** The 'Receive Provider Information from Primary Health Care Provider Data Store' activity is completed successfully and receive external provider data through standard integration while retaining source and error information, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Primary Health Care Provider Data Store; Provider Directory; Provider Identity / Matching Service
