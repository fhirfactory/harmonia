# Validate Clinician Information

## Purpose

incomplete or invalid provider records are identified

## Source Traceability

- **Source Use Case:** `UC-Integration-ProviderDirectory-3`
- **Source Scope:** Scope - Provider Registration and Onboarding
- **Primary Actor:** Provider Directory Validation Service
- **Supporting Actors:** Provider Directory
- **User Story:** Validate submitted practitioner information so that incomplete or invalid provider records are identified.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Validate Clinician Information
Harmonia SHALL support **Validate Clinician Information** so that incomplete or invalid provider records are identified.

#### Scenario: Validate Clinician Information - successful outcome
...
- **WHEN** The actor needs to perform the 'Validate Clinician Information' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory Validation Service initiates the 'Validate Clinician Information' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Provider Directory Validation Service enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Validate Clinician Information' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Provider Directory Validation Service
