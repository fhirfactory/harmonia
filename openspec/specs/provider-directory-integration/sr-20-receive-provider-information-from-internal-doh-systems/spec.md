# Receive Provider Information from Internal DoH Systems

## Purpose

Enable the Provider Directory to receive provider information from internal DoH systems through standard integration and retain source and error information. This supports the target-state objective that sr-20 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the directory of internal providers is kept up to date.

## Source Traceability

- **Source Use Case:** `SR-20`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Department of Health Source System; Provider Directory; Provider Identity / Matching Service
- **User Story:** As an Integration Administrator, I want the Provider Directory to receive provider information from internal DoH systems, so that the directory of internal providers is kept up to date.
...
### Requirement: Receive Provider Information from Internal DoH Sy
Harmonia SHALL support **Receive Provider Information from Internal DoH Systems** so that Enable the Provider Directory to receive provider information from internal DoH systems through standard integration and retain source and error information. This supports the target-state objective that sr-20 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the directory of internal providers is kept up to date.

#### Scenario: Receive Provider Information from Internal DoH Systems - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Receive Provider Information from Internal DoH Systems' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Receive Provider Information from Internal DoH Systems' activity.
...
- **THEN** 6. The Provider Directory performs the required action to receive provider information from internal DoH systems through standard integration and retain source and error information.
...
- **AND** supporting participation may be provided by Department of Health Source System; Provider Directory; Provider Identity / Matching Service
