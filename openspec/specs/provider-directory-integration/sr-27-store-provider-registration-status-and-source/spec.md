# Store Provider Registration Status and Source

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-27 so that the directory contains an accurate view of who is registered to practice. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-27`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Compliance Officer
- **Supporting Actors:** Provider Directory Administrator; National Provider Data Store; Provider Directory
- **User Story:** As a Compliance Officer, I want the provider directory to store the registration status and source, so that the directory contains an accurate view of who is registered to practice.
...
### Requirement: Store Provider Registration Status and Source
Harmonia SHALL support **Store Provider Registration Status and Source** so that Enable the target-state Provider Directory to support the capability described by SR-27 so that the directory contains an accurate view of who is registered to practice. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Store Provider Registration Status and Source - successful outcome
...
- **WHEN** The Compliance Officer initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-27.
- **THEN** 1. The Compliance Officer initiates or receives the event associated with “Store Provider Registration Status and Source”.
...
- **THEN** The requirement described by SR-27 is completed successfully so that the directory contains an accurate view of who is registered to practice. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Provider Directory Administrator; National Provider Data Store; Provider Directory
...
- **WHEN** The data source is recorded whenever registration status is updated.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** The history of registration status changes can be viewed.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** A non-active registration status can trigger a workflow or notification.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Compliance reporting includes registration status changes.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** The data source is recorded whenever registration status is updated.
...
- **GIVEN** The history of registration status changes can be viewed.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** A non-active registration status can trigger a workflow or notification.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Compliance reporting includes registration status changes.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
