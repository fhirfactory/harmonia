# Receive Care Provider Details from Department of Health Systems

## Purpose

Enable the Provider Directory to receive provider information from internal Department of Health systems. This supports the target-state objective that uc-4 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the directory of internal providers is kept current using controlled system integration.

## Source Traceability

- **Source Use Case:** `UC-4`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Department of Health Source System
- **Supporting Actors:** Integration Administrator; Provider Directory; Provider Identity / Matching Service
- **User Story:** As the Provider Directory, I need to receive provider information from internal Department of Health systems, so that the directory of internal providers is kept current using controlled system integration.
...
### Requirement: Receive Care Provider Details from Department of
Harmonia SHALL support **Receive Care Provider Details from Department of Health Systems** so that Enable the Provider Directory to receive provider information from internal Department of Health systems. This supports the target-state objective that uc-4 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that the directory of internal providers is kept current using controlled system integration.

#### Scenario: Receive Care Provider Details from Department of Health Systems - successful outcome
...
- **WHEN** The Department of Health Source System initiates the 'Receive Care Provider Details from Department of Health Systems' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Department of Health Source System initiates or causes the 'Receive Care Provider Details from Department of Health Systems' activity.
...
- **THEN** 6. The Provider Directory performs the required action to receive provider information from internal Department of Health systems.
...
- **THEN** The 'Receive Care Provider Details from Department of Health Systems' activity is completed successfully and receive provider information from internal Department of Health systems, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Department of Health Source System
