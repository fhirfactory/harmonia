# Manually Create New Provider Records

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-47 so that missing providers can be added to the directory when needed. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-47`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Provider Identity / Matching Service; Data Steward; Provider Directory
- **User Story:** As a Provider Directory Administrator, I want to manually create new provider records, so that missing providers can be added to the directory when needed.
...
### Requirement: Manually Create New Provider Records
Harmonia SHALL support **Manually Create New Provider Records** so that Enable the target-state Provider Directory to support the capability described by SR-47 so that missing providers can be added to the directory when needed. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Manually Create New Provider Records - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-47.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Manually Create New Provider Records”.
...
- **THEN** The requirement described by SR-47 is completed successfully so that missing providers can be added to the directory when needed. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Provider Identity / Matching Service; Data Steward; Provider Directory
...
- **WHEN** New provider records can be created manually through a user interface.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Potential duplicates can be identified before a new record is created.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Records created through the user interface are marked as manually created.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Creation of new records can trigger notifications where configured.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** New provider records can be created manually through a user interface.
...
- **GIVEN** Potential duplicates can be identified before a new record is created.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Records created through the user interface are marked as manually created.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Creation of new records can trigger notifications where configured.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
