# Monitor Provider Directory API Feeds

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-43 so that I can ensure data is delivered reliably and securely. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-43`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Integration Administrator; API Management Service; Provider Directory; Integration Service
- **User Story:** As a Provider Directory Administrator, I want to monitor the API feeds, so that I can ensure data is delivered reliably and securely.
...
### Requirement: Monitor Provider Directory API Feeds
Harmonia SHALL support **Monitor Provider Directory API Feeds** so that Enable the target-state Provider Directory to support the capability described by SR-43 so that I can ensure data is delivered reliably and securely. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Monitor Provider Directory API Feeds - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-43.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Monitor Provider Directory API Feeds”.
...
- **THEN** The requirement described by SR-43 is completed successfully so that I can ensure data is delivered reliably and securely. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Integration Administrator; API Management Service; Provider Directory; Integration Service
...
- **WHEN** API usage statistics can be reported, including volume, peaks, frequency, consumers and provider types.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Abnormal or excessive API usage can generate a notification.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** API performance can be reported and monitored.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** API usage statistics can be reported, including volume, peaks, frequency, consumers and provider types.
...
- **GIVEN** Abnormal or excessive API usage can generate a notification.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** API performance can be reported and monitored.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
