...
- **User Story:** As a Provider Directory Administrator, I want to generate reports that show missing or incomplete provider data, so that I can fix gaps in the directory.
...
### Requirement: Report Missing or Incomplete Provider Data
Harmonia SHALL support **Report Missing or Incomplete Provider Data** so that Enable the target-state Provider Directory to support the capability described by SR-49 so that I can fix gaps in the directory. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Report Missing or Incomplete Provider Data - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-49.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Report Missing or Incomplete Provider Data”.
...
- **THEN** The requirement described by SR-49 is completed successfully so that I can fix gaps in the directory. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **WHEN** Reports can identify provider records missing specified fields.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Reports can identify records with any missing data where appropriate.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Required-field rules are configurable.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Reports can identify provider records missing specified fields.
...
- **GIVEN** Reports can identify records with any missing data where appropriate.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Required-field rules are configurable.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
