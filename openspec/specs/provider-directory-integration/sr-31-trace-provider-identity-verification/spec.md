# Trace Provider Identity Verification

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-31 so that I can support audits or investigate discrepancies. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-31`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Compliance Officer; Provider Identity / Matching Service; Provider Directory
- **User Story:** As a Provider Directory Administrator, I want to be able to trace how a provider’s identity was confirmed, so that I can support audits or investigate discrepancies.
...
### Requirement: Trace Provider Identity Verification
Harmonia SHALL support **Trace Provider Identity Verification** so that Enable the target-state Provider Directory to support the capability described by SR-31 so that I can support audits or investigate discrepancies. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Trace Provider Identity Verification - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-31.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Trace Provider Identity Verification”.
...
- **THEN** The requirement described by SR-31 is completed successfully so that I can support audits or investigate discrepancies. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Compliance Officer; Provider Identity / Matching Service; Provider Directory
...
- **WHEN** An audit trail records how provider identity was confirmed.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** The source of the identity check, such as the HI Service, can be viewed for each provider record.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Identity verification history can be traced by provider.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 12
- **WHEN** Manual overrides are recorded with justification.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** An audit trail records how provider identity was confirmed.
...
- **GIVEN** The source of the identity check, such as the HI Service, can be viewed for each provider record.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Identity verification history can be traced by provider.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 4
- **GIVEN** Manual overrides are recorded with justification.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
