# Expose Reference Data Sets via an API

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-58 so that external systems can consume standardised values. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-58`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Integration Administrator; Downstream Consumer System; Provider Directory; Integration Service
- **User Story:** As a Provider Directory Administrator, I want to expose reference data sets via an API, so that external systems can consume standardised values.
- **Related Use Cases:** UC-Integration-ProviderDirectory-35 – Monitor Provider Integration Interfaces: supports operational monitoring of Provider Directory interfaces.; UC-Integration-ProviderDirectory-36 – Reconcile Provider Synchronisation Failures: supports remediation of failed or incomplete synchronisation.; UC-Integration-ProviderDirectory-32 – Validate Provider Data Quality: supports validation, data-quality and remediation controls.; Cross References

## Requirements

### Requirement: Expose Reference Data Sets via an API
Harmonia SHALL support **Expose Reference Data Sets via an API** so that Enable the target-state Provider Directory to support the capability described by SR-58 so that external systems can consume standardised values. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Expose Reference Data Sets via an API - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-58.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Expose Reference Data Sets via an API”.
...
- **THEN** The requirement described by SR-58 is completed successfully so that external systems can consume standardised values. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Integration Administrator; Downstream Consumer System; Provider Directory; Integration Service
...
- **WHEN** Reference data sets can be exposed through an API.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Only approved reference data sets are exposed through the API.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** API documentation describes the structure and intended use of each exposed reference data set.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Reference data sets can be exposed through an API.
...
- **GIVEN** Only approved reference data sets are exposed through the API.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** API documentation describes the structure and intended use of each exposed reference data set.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
