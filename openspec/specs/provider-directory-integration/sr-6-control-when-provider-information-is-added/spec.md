# Control When Provider Information Is Added

## Purpose

Enable the Provider Directory to schedule, trigger, pause and resume incoming provider feeds. This supports the target-state objective that sr-6 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can effectively manage system resources.

## Source Traceability

- **Source Use Case:** `SR-6`
...
### Requirement: Control When Provider Information Is Added
Harmonia SHALL support **Control When Provider Information Is Added** so that Enable the Provider Directory to schedule, trigger, pause and resume incoming provider feeds. This supports the target-state objective that sr-6 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that I can effectively manage system resources.

#### Scenario: Control When Provider Information Is Added - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Control When Provider Information Is Added' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Control When Provider Information Is Added' activity.
...
- **THEN** 6. The Provider Directory performs the required action to schedule, trigger, pause and resume incoming provider feeds.
...
- **THEN** The 'Control When Provider Information Is Added' activity is completed successfully and schedule, trigger, pause and resume incoming provider feeds, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
