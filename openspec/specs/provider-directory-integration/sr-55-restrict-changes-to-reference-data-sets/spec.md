# Restrict Changes to Reference Data Sets

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-55 so that unapproved changes are prevented. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-55`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Security / Access Management Service; Compliance Officer; Provider Directory
- **User Story:** As a Provider Directory Administrator, I want to limit changes to reference data sets to specific roles only, so that unapproved changes are prevented.
...
### Requirement: Restrict Changes to Reference Data Sets
Harmonia SHALL support **Restrict Changes to Reference Data Sets** so that Enable the target-state Provider Directory to support the capability described by SR-55 so that unapproved changes are prevented. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Restrict Changes to Reference Data Sets - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-55.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Restrict Changes to Reference Data Sets”.
...
- **THEN** The requirement described by SR-55 is completed successfully so that unapproved changes are prevented. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Security / Access Management Service; Compliance Officer; Provider Directory
...
- **WHEN** Permissions to create, edit or delete reference data sets can be assigned to specific users or roles.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** All reference data changes are audited.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Reference data values can be locked or otherwise protected from unauthorised change.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Permissions to create, edit or delete reference data sets can be assigned to specific users or roles.
...
- **GIVEN** All reference data changes are audited.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Reference data values can be locked or otherwise protected from unauthorised change.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
