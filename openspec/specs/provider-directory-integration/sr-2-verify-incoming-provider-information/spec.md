# Verify Incoming Provider Information

## Purpose

Enable the Provider Directory to validate formats, mandatory fields and reference values and transform data to defined standards before acceptance. This supports the target-state objective that sr-2 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incorrect records are not stored.

## Source Traceability

- **Source Use Case:** `SR-2`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Integration Administrator
- **Supporting Actors:** Data Steward; Provider Directory; Provider Identity / Matching Service
- **User Story:** As an Integration Administrator, I want to verify provider information, so that incorrect records are not stored.
...
### Requirement: Verify Incoming Provider Information
Harmonia SHALL support **Verify Incoming Provider Information** so that Enable the Provider Directory to validate formats, mandatory fields and reference values and transform data to defined standards before acceptance. This supports the target-state objective that sr-2 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that incorrect records are not stored.

#### Scenario: Verify Incoming Provider Information - successful outcome
...
- **WHEN** The Integration Administrator initiates the 'Verify Incoming Provider Information' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Integration Administrator initiates or causes the 'Verify Incoming Provider Information' activity.
...
- **THEN** 6. The Provider Directory performs the required action to validate formats, mandatory fields and reference values and transform data to defined standards before acceptance.
...
- **THEN** The 'Verify Incoming Provider Information' activity is completed successfully and validate formats, mandatory fields and reference values and transform data to defined standards before acceptance, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **AND** supporting participation may be provided by Data Steward; Provider Directory; Provider Identity / Matching Service
