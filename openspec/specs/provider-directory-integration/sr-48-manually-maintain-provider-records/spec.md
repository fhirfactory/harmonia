...
- **User Story:** As a Provider Directory Administrator, I want to manually maintain provider records, so that provider information can be kept accurate.
...
### Requirement: Manually Maintain Provider Records
Harmonia SHALL support **Manually Maintain Provider Records** so that Enable the target-state Provider Directory to support the capability described by SR-48 so that provider information can be kept accurate. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Manually Maintain Provider Records - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-48.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Manually Maintain Provider Records”.
...
- **THEN** The requirement described by SR-48 is completed successfully so that provider information can be kept accurate. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **WHEN** Provider records can be maintained manually through a user interface.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Manual changes are identified as such in the record history.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 11
- **WHEN** Updated records can trigger notifications where configured.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Provider records can be maintained manually through a user interface.
...
- **GIVEN** Manual changes are identified as such in the record history.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint

#### Scenario: Business or data control 3
- **GIVEN** Updated records can trigger notifications where configured.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
