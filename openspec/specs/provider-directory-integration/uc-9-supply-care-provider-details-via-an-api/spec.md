# Supply Care Provider Details via an API

## Purpose

Enable the Provider Directory to make provider information available through a governed API. This supports the target-state objective that uc-9 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that authorised systems can retrieve current provider information using a reusable machine-to-machine interface.

## Source Traceability

- **Source Use Case:** `UC-9`
- **Source Scope:** Additional Source Use Cases and System Requirements
- **Primary Actor:** Downstream Consumer System
- **Supporting Actors:** Integration Administrator; Provider Directory
- **User Story:** As the Provider Directory, I need to make provider information available through a governed API, so that authorised systems can retrieve current provider information using a reusable machine-to-machine interface.
...
### Requirement: Supply Care Provider Details via an API
Harmonia SHALL support **Supply Care Provider Details via an API** so that Enable the Provider Directory to make provider information available through a governed API. This supports the target-state objective that uc-9 is implemented as a governed business capability rather than as an isolated technical function. The intended outcome is that authorised systems can retrieve current provider information using a reusable machine-to-machine interface.

#### Scenario: Supply Care Provider Details via an API - successful outcome
...
- **WHEN** The Downstream Consumer System initiates the 'Supply Care Provider Details via an API' activity, a relevant provider-data event occurs, or an authorised scheduled process initiates the activity.
- **THEN** 1. The Downstream Consumer System initiates or causes the 'Supply Care Provider Details via an API' activity.
...
- **THEN** 6. The Provider Directory performs the required action to make provider information available through a governed API.
...
- **THEN** The 'Supply Care Provider Details via an API' activity is completed successfully and make provider information available through a governed API, while maintaining provider identity, provenance, data quality, governance and auditability. Where the activity cannot be completed automatically, a clear validation, exception, reconciliation or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Downstream Consumer System
