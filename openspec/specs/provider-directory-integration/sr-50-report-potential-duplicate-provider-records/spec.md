# Report Potential Duplicate Provider Records

## Purpose

Enable the target-state Provider Directory to support the capability described by SR-50 so that I can investigate and resolve them. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

## Source Traceability

- **Source Use Case:** `SR-50`
- **Source Scope:** Additional Source Requirements – SR-26 to SR-58
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Data Steward; Provider Identity / Matching Service; Provider Directory
- **User Story:** As a Provider Directory Administrator, I want to generate reports that identify potential duplicate provider records, so that I can investigate and resolve them.
- **Related Use Cases:** UC-Integration-ProviderDirectory-30 – Match Provider Records: supports provider identity resolution and duplicate avoidance.; UC-Integration-ProviderDirectory-31 – Receive Provider Identifier Cross References: supports reconciliation of identifiers across source systems.; UC-Integration-ProviderDirectory-32 – Validate Provider Data Quality: supports validation, data-quality and remediation controls.

## Requirements

### Requirement: Report Potential Duplicate Provider Records
Harmonia SHALL support **Report Potential Duplicate Provider Records** so that Enable the target-state Provider Directory to support the capability described by SR-50 so that I can investigate and resolve them. The capability should operate as part of the governed Provider Directory service, with consistent identity, provenance, lifecycle, security, data-quality and audit controls applied irrespective of whether the activity is performed manually, through an API, or through system integration.

#### Scenario: Report Potential Duplicate Provider Records - successful outcome
...
- **WHEN** The Provider Directory Administrator initiates the activity, a relevant provider or reference-data change occurs, or an authorised scheduled/event-driven process triggers the requirement described by SR-50.
- **THEN** 1. The Provider Directory Administrator initiates or receives the event associated with “Report Potential Duplicate Provider Records”.
...
- **THEN** The requirement described by SR-50 is completed successfully so that I can investigate and resolve them. The Provider Directory maintains an auditable and governed record of the activity and any exceptions are clearly identified for follow-up.
...
- **AND** supporting participation may be provided by Data Steward; Provider Identity / Matching Service; Provider Directory
...
- **WHEN** Potential duplicates can be identified using demographic or other approved matching data.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state

#### Scenario: Exception 10
- **WHEN** Potential duplicates are flagged for review with a reason or match basis.
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
...
- **GIVEN** Potential duplicates can be identified using demographic or other approved matching data.
...
- **GIVEN** Potential duplicates are flagged for review with a reason or match basis.
- **WHEN** information is created, changed, validated, published, exchanged or consumed through this capability
- **THEN** Harmonia SHALL preserve the stated control as an observable business rule or constraint
