# Manage Clinician Role History

## Purpose

previous organisational relationships remain auditable

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-28`
- **Source Scope:** Scope - Advanced Enterprise Provider Directory
- **Primary Actor:** Provider Directory Administrator (Workforce)
- **Supporting Actors:** Provider Directory
- **User Story:** Maintain historical practitioner roles and affiliations so that previous organisational relationships remain auditable.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Manage Clinician Role History
Harmonia SHALL support **Manage Clinician Role History** so that previous organisational relationships remain auditable.

#### Scenario: Manage Clinician Role History - successful outcome
...
- **WHEN** The actor needs to perform the 'Manage Clinician Role History' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory Administrator (Workforce) initiates the 'Manage Clinician Role History' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Provider Directory Administrator (Workforce) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Manage Clinician Role History' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Provider Directory Administrator (Workforce)
