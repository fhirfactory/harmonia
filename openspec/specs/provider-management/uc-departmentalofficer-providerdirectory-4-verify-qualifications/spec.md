...
professional credentials are accurately represented

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-4`
- **Source Scope:** Scope - Clinician Registration
- **Primary Actor:** Credentialing Officer
- **Supporting Actors:** Provider Directory
- **User Story:** Validate a practitioner's qualifications so that professional credentials are accurately represented.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Verify Qualifications
Harmonia SHALL support **Verify Qualifications** so that professional credentials are accurately represented.

#### Scenario: Verify Qualifications - successful outcome
...
- **WHEN** The actor needs to perform the 'Verify Qualifications' activity, or an authorised business process initiates the activity.
- **THEN** Credentialing Officer initiates the 'Verify Qualifications' activity.
...
- **THEN** The 'Verify Qualifications' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
