...
referrals are practical and accessible

## Source Traceability

- **Source Use Case:** `UC-Clinician-ProviderDirectorySearch-11`
- **Source Scope:** Scope - Location and Accessibility-Based Search
- **Primary Actor:** Clinician
- **Supporting Actors:** Provider Directory
- **User Story:** Find providers near a participant's location so that referrals are practical and accessible.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Search Providers by Location
Harmonia SHALL support **Search Providers by Location** so that referrals are practical and accessible.

#### Scenario: Search Providers by Location - successful outcome
...
- **WHEN** The actor needs to perform the 'Search Providers by Location' activity, or an authorised business process initiates the activity.
- **THEN** Clinician initiates the 'Search Providers by Location' activity.
...
- **THEN** The 'Search Providers by Location' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
