...
renewals can be obtained before expiry

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-14`
- **Source Scope:** Scope - Credentials and Authorisations
- **Primary Actor:** Credentialing Officer
- **Supporting Actors:** Provider Directory
- **User Story:** Identify clinicians with expiring registrations or licences so that renewals can be obtained before expiry.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Monitor Credential Expiry
Harmonia SHALL support **Monitor Credential Expiry** so that renewals can be obtained before expiry.

#### Scenario: Monitor Credential Expiry - successful outcome
...
- **WHEN** The actor needs to perform the 'Monitor Credential Expiry' activity, or an authorised business process initiates the activity.
- **THEN** Credentialing Officer initiates the 'Monitor Credential Expiry' activity.
...
- **THEN** The 'Monitor Credential Expiry' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
