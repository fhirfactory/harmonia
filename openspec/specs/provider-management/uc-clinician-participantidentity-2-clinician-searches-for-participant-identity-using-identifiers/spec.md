# Clinician Searches for Participant Identity using Identifiers

## Purpose

Search for a given Participant's Identity using regulatory or enterprise-unique identifiers

## Source Traceability

- **Source Use Case:** `UC-Clinician-ParticipantIdentity-2`
- **Source Scope:** Scope – Clinician Access/Use to Participant Identity
- **Primary Actor:** Clinician
- **Supporting Actors:** Provider Directory
- **User Story:** Search for a given Participant's Identity using regulatory or enterprise-unique identifiers.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Clinician Searches for Participant Identity using
Harmonia SHALL support **Clinician Searches for Participant Identity using Identifiers** so that Search for a given Participant's Identity using regulatory or enterprise-unique identifiers.

#### Scenario: Clinician Searches for Participant Identity using Identifiers - successful outcome
...
- **WHEN** The actor needs to perform the 'Clinician Searches for Participant Identity using Identifiers' activity, or an authorised business process initiates the activity.
- **THEN** Clinician initiates the 'Clinician Searches for Participant Identity using Identifiers' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Clinician enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Clinician Searches for Participant Identity using Identifiers' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Clinician
