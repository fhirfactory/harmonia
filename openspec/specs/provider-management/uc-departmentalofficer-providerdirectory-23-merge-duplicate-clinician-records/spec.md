# Merge Duplicate Clinician Records

## Purpose

each practitioner has a single authoritative profile

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-23`
- **Source Scope:** Scope – Provider Directory
- **Primary Actor:** Data Steward
- **Supporting Actors:** Provider Directory
- **User Story:** Merge duplicate practitioner records so that each practitioner has a single authoritative profile.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Merge Duplicate Clinician Records
Harmonia SHALL support **Merge Duplicate Clinician Records** so that each practitioner has a single authoritative profile.

#### Scenario: Merge Duplicate Clinician Records - successful outcome
...
- **WHEN** The actor needs to perform the 'Merge Duplicate Clinician Records' activity, or an authorised business process initiates the activity.
- **THEN** Data Steward initiates the 'Merge Duplicate Clinician Records' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Data Steward enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Merge Duplicate Clinician Records' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Data Steward
