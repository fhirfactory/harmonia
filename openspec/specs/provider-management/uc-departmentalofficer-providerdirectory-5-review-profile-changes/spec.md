...
only accurate information is published

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-5`
- **Source Scope:** Scope - Profile Management
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Provider Directory
- **User Story:** Review practitioner profile updates so that only accurate information is published.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Review Profile Changes
Harmonia SHALL support **Review Profile Changes** so that only accurate information is published.

#### Scenario: Review Profile Changes - successful outcome
...
- **WHEN** The actor needs to perform the 'Review Profile Changes' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory Administrator initiates the 'Review Profile Changes' activity.
...
- **THEN** The 'Review Profile Changes' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
