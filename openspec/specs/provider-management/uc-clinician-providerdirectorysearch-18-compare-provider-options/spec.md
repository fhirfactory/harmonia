...
I can select the most appropriate service for a participant

## Source Traceability

- **Source Use Case:** `UC-Clinician-ProviderDirectorySearch-18`
- **Source Scope:** Scope - Availability and Referral Suitability
- **Primary Actor:** Clinician (Care Coordinator)
- **Supporting Actors:** Provider Directory
- **User Story:** Compare suitable provider options so that I can select the most appropriate service for a participant.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Compare Provider Options
Harmonia SHALL support **Compare Provider Options** so that I can select the most appropriate service for a participant.

#### Scenario: Compare Provider Options - successful outcome
...
- **WHEN** The actor needs to perform the 'Compare Provider Options' activity, or an authorised business process initiates the activity.
- **THEN** Clinician (Care Coordinator) initiates the 'Compare Provider Options' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Clinician (Care Coordinator) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Compare Provider Options' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Clinician (Care Coordinator)
...
- **WHEN** Actor – Participant / Patient
- **THEN** Harmonia SHALL prevent an invalid or unauthorised outcome and record or present the applicable correction, approval, reconciliation, escalation or failure state
