...
only legitimate healthcare providers are registered

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-2`
...
- **User Story:** Verify the identity of a practitioner so that only legitimate healthcare providers are registered.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Verify Clinician Identity
Harmonia SHALL support **Verify Clinician Identity** so that only legitimate healthcare providers are registered.

#### Scenario: Verify Clinician Identity - successful outcome
...
- **WHEN** The actor needs to perform the 'Verify Clinician Identity' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory Administrator (Registration) initiates the 'Verify Clinician Identity' activity.
...
- **THEN** The 'Verify Clinician Identity' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
