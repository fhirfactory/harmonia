...
a healthcare provider can be recognised within the Provider Directory

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-1`
- **Source Scope:** Scope - Clinician Registration
- **Primary Actor:** Provider Directory Administrator (Provider Registration)
- **Supporting Actors:** Provider Directory
- **User Story:** Create a practitioner record so that a healthcare provider can be recognised within the Provider Directory.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Register a Clinician
Harmonia SHALL support **Register a Clinician** so that a healthcare provider can be recognised within the Provider Directory.

#### Scenario: Register a Clinician - successful outcome
...
- **WHEN** The actor needs to perform the 'Register a Clinician' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory Administrator (Provider Registration) initiates the 'Register a Clinician' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Provider Directory Administrator (Provider Registration) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Register a Clinician' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Provider Directory Administrator (Provider Registration)
