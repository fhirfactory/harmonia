# Search for a Provider by Clinical Need

## Purpose

I can identify an appropriate provider to support the participant's care

## Source Traceability

- **Source Use Case:** `UC-Clinician-ProviderDirectorySearch-1`
- **Source Scope:** Scope - Clinician Search (Provider Directory)
- **Primary Actor:** Clinician (General Practitioner)
- **Supporting Actors:** Provider Directory
- **User Story:** Search the Provider Directory using a participant's clinical problem or healthcare need so that I can identify an appropriate provider to support the participant's care.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Search for a Provider by Clinical Need
Harmonia SHALL support **Search for a Provider by Clinical Need** so that I can identify an appropriate provider to support the participant's care.

#### Scenario: Search for a Provider by Clinical Need - successful outcome
...
- **WHEN** The actor needs to perform the 'Search for a Provider by Clinical Need' activity, or an authorised business process initiates the activity.
- **THEN** Clinician (General Practitioner) initiates the 'Search for a Provider by Clinical Need' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Clinician (General Practitioner) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Search for a Provider by Clinical Need' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is Clinician (General Practitioner)
