# Suspend Clinician Listings

## Purpose

only authorised clinicians appear in the directory

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-15`
- **Source Scope:** Scope - Credentials and Authorisations
- **Primary Actor:** Compliance Officer
- **Supporting Actors:** Provider Directory
- **User Story:** Suspend practitioner listings when professional registration lapses or disciplinary action occurs so that only authorised clinicians appear in the directory.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Suspend Clinician Listings
Harmonia SHALL support **Suspend Clinician Listings** so that only authorised clinicians appear in the directory.

#### Scenario: Suspend Clinician Listings - successful outcome
...
- **WHEN** The actor needs to perform the 'Suspend Clinician Listings' activity, or an authorised business process initiates the activity.
- **THEN** Compliance Officer initiates the 'Suspend Clinician Listings' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** Compliance Officer enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Suspend Clinician Listings' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
