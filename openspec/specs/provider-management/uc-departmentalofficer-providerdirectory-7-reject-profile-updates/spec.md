...
inaccurate information is not published

## Source Traceability

- **Source Use Case:** `UC-DepartmentalOfficer-ProviderDirectory-7`
- **Source Scope:** Scope - Profile Management
- **Primary Actor:** Provider Directory Administrator
- **Supporting Actors:** Provider Directory
- **User Story:** Reject profile changes that cannot be verified so that inaccurate information is not published.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Reject Profile Updates
Harmonia SHALL support **Reject Profile Updates** so that inaccurate information is not published.

#### Scenario: Reject Profile Updates - successful outcome
...
- **WHEN** The actor needs to perform the 'Reject Profile Updates' activity, or an authorised business process initiates the activity.
- **THEN** Provider Directory Administrator initiates the 'Reject Profile Updates' activity.
...
- **THEN** The 'Reject Profile Updates' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
