...
The Clinician’s current information is available in the Provider Directory

## Source Traceability

- **Source Use Case:** `UC-Clinician-Directory-20`
- **Source Scope:** Scope – Clinician Availability and Visibility
- **User Story:** Publish approved changes to my profile so that current information is available in the Provider Directory.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Publish My Profile
Harmonia SHALL support **Publish My Profile** so that The Clinician’s current information is available in the Provider Directory.

#### Scenario: Publish My Profile - successful outcome
...
- **WHEN** The actor needs to perform the 'Publish My Profile' activity, or an authorised business process initiates the activity.
- **THEN** Clinician (Registered) initiates the 'Publish My Profile' activity.
...
- **THEN** The 'Publish My Profile' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
