# Confirm Participant Capacity

## Purpose

consent decisions are valid

## Source Traceability

- **Source Use Case:** `UC-Clinician-Consent-6`
...
- **User Story:** Record whether the participant has the capacity to provide informed consent so that consent decisions are valid.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Confirm Participant Capacity
Harmonia SHALL support **Confirm Participant Capacity** so that consent decisions are valid.

#### Scenario: Confirm Participant Capacity - successful outcome
...
- **WHEN** The actor needs to perform the 'Confirm Participant Capacity' activity, or an authorised business process initiates the activity.
- **THEN** Clinician (Treating Clinician) initiates the 'Confirm Participant Capacity' activity.
...
- **THEN** The 'Confirm Participant Capacity' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
