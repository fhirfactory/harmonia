# Record Representative Consent

## Purpose

treatment decisions are appropriately documented

## Source Traceability

- **Source Use Case:** `UC-ExternalClinician-Consent-14`
- **Source Scope:** Scope - Representative Consent
- **Primary Actor:** External ClinicianProvider (Paediatrician)
- **Supporting Actors:** Provider Directory
- **User Story:** Record consent provided by a participant's authorised representative so that treatment decisions are appropriately documented.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Record Representative Consent
Harmonia SHALL support **Record Representative Consent** so that treatment decisions are appropriately documented.

#### Scenario: Record Representative Consent - successful outcome
...
- **WHEN** The actor needs to perform the 'Record Representative Consent' activity, or an authorised business process initiates the activity.
- **THEN** External ClinicianProvider (Paediatrician) initiates the 'Record Representative Consent' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** External ClinicianProvider (Paediatrician) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Record Representative Consent' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is External ClinicianProvider (Paediatrician)
