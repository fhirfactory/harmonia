...
the authorisation is available to all participating organisations

## Source Traceability

- **Source Use Case:** `UC-ExternalClinician-Consent-6`
- **Source Scope:** Scope - Obtaining Consent
- **Primary Actor:** External ClinicianProvider (General Practitioner)
- **Supporting Actors:** Provider Directory
- **User Story:** Record the participant's consent so that the authorisation is available to all participating organisations.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Record Participant Consent
Harmonia SHALL support **Record Participant Consent** so that the authorisation is available to all participating organisations.

#### Scenario: Record Participant Consent - successful outcome
...
- **WHEN** The actor needs to perform the 'Record Participant Consent' activity, or an authorised business process initiates the activity.
- **THEN** External ClinicianProvider (General Practitioner) initiates the 'Record Participant Consent' activity.
...
- **THEN** The 'Record Participant Consent' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
