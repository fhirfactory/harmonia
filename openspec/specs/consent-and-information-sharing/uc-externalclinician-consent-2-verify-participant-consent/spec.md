...
I comply with privacy and legislative requirements

## Source Traceability

- **Source Use Case:** `UC-ExternalClinician-Consent-2`
- **Source Scope:** Scope - Clinician Access to Participant Information
- **Primary Actor:** External ClinicianProvider (Allied Health Clinician)
- **Supporting Actors:** Provider Directory
- **User Story:** Verify that the participant has authorised me to access their information so that I comply with privacy and legislative requirements.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Verify Participant Consent
Harmonia SHALL support **Verify Participant Consent** so that I comply with privacy and legislative requirements.

#### Scenario: Verify Participant Consent - successful outcome
...
- **WHEN** The actor needs to perform the 'Verify Participant Consent' activity, or an authorised business process initiates the activity.
- **THEN** External ClinicianProvider (Allied Health Clinician) initiates the 'Verify Participant Consent' activity.
- **THEN** The solution presents the information and/or controls required to perform the activity described by the user story.
- **THEN** External ClinicianProvider (Allied Health Clinician) enters, selects, reviews or confirms the relevant information.
...
- **THEN** The 'Verify Participant Consent' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
...
- **GIVEN** the primary actor is External ClinicianProvider (Allied Health Clinician)
