...
- **Source Use Case:** `UC-ExternalClinician-Consent-18`
...
- **User Story:** Record when emergency access has been exercised so that the access is auditable.
- **Related Use Cases:** None identified.

## Requirements

### Requirement: Record Emergency Access
Harmonia SHALL support **Record Emergency Access** so that the access is auditable.

#### Scenario: Record Emergency Access - successful outcome
...
- **WHEN** The actor needs to perform the 'Record Emergency Access' activity, or an authorised business process initiates the activity.
- **THEN** External ClinicianProvider (Urgent Care Clinician) initiates the 'Record Emergency Access' activity.
...
- **THEN** The 'Record Emergency Access' activity is completed successfully, or a clear validation, approval, exception or escalation outcome is recorded.
