...
/**
 * Policy governing asynchronous workflow execution and task processing for Provider Registry changes.
 */
public class ProviderRegistryProcessPolicy implements ThemisPolicy {

    public static final String POLICY_ID = "provider-registry-process-policy";
    ...
    public String getDescription() {
        return "Authorizes PROCESS and EXECUTE actions on Provider Registry tasks within Ponos and Erga.";
    }
    ...
    public boolean appliesTo(ThemisAuthorizationRequest request) {
        ...
        boolean matchesAction = request.action() == ThemisAction.PROCESS || request.action() == ThemisAction.EXECUTE;
    ...
    public ThemisAuthorizationDecision evaluate(ThemisAuthorizationRequest request) {
        ...
        if (request.hasAuthority(AUTH_PROVIDER_ADMIN) || request.hasAuthority(AUTH_PROVIDER_CHANGE_PROCESS)) {
            return ThemisAuthorizationDecision.allow(POLICY_ID, corrId, "Authorized by provider.change.process");
        }
        return ThemisAuthorizationDecision.deny(ThemisDecisionReason.EXECUTION_AUTHORITY_MISSING, POLICY_ID, corrId, "Missing required execution authority: " + AUTH_PROVIDER_CHANGE_PROCESS);
    }
}
