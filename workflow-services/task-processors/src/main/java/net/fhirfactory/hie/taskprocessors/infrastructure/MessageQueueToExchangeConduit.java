 ...
 * <p>
 * Ingests incoming message payloads (such as {@link TaskEvent} instances, JSON documents,
 ...
 */
@Dependent
public class MessageQueueToExchangeConduit extends TaskProcessingActivity {
    ...
    public void processMessageQueueToExchange(Exchange exchange) {
        ...
        if (body instanceof TaskEvent) {
            processTaskEvent(exchange, (TaskEvent) body);
    ...
    private void processTaskEvent(Exchange exchange, TaskEvent taskEvent) {
        if (taskEvent == null) {
            return;
        }

        setHeaderIfAbsent(exchange, HEADER_GATEWAY_INSTANCE, taskEvent.getGatewayInstanceId());
        setHeaderIfAbsent(exchange, HEADER_TRIGGER_TYPE, taskEvent.getTriggerType());
        setHeaderIfAbsent(exchange, HEADER_MESSAGE_TYPE, taskEvent.getMessageType());
        setHeaderIfAbsent(exchange, HEADER_TASK_ID, taskEvent.getTaskId());
        setHeaderIfAbsent(exchange, HEADER_CONTROL_ID, taskEvent.getControlId());
        setHeaderIfAbsent(exchange, HEADER_ACTION, taskEvent.getAction());
        setHeaderIfAbsent(exchange, HEADER_STATUS, taskEvent.getStatus());

        if (taskEvent.getTopic() != null) {
            Topic t = taskEvent.getTopic();
        ...
        if (exchange.getMessage().getHeader(HEADER_RAW_MESSAGE) == null) {
            try {
                String json = objectMapper.writeValueAsString(taskEvent);
