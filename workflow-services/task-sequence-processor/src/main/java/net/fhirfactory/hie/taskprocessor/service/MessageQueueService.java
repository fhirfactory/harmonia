...
/**
 * Service managing persisted {@link MessageQueueDefinition} configurations in the Infinispan cluster
...
public class MessageQueueService {
    ...
    public String toJson(MessageQueueDefinition queueDef) {
        if (queueDef == null) {
            return null;
        }
        try {
    ...
    public MessageQueueDefinition fromJson(String json) {
        if (StringUtils.isBlank(json)) {
            return null;
        }
        try {
            return getObjectMapper().readValue(json, MessageQueueDefinition.class);
        } catch (Exception e) {
    ...
    public MessageQueueDefinition save(MessageQueueDefinition queueDef) {
        if (queueDef == null) {
    ...
    public Optional<MessageQueueDefinition> getById(String queueId) {
        if (StringUtils.isBlank(queueId)) {
    ...
    public List<MessageQueueDefinition> getAll() {
        Map<String, MessageQueueDefinition> resultMap = new LinkedHashMap<>();
            ...
            try {
                ...
                if (values != null && !values.isEmpty()) {
                    for (String json : values) {
                        MessageQueueDefinition def = fromJson(json);
        ...
        for (Map.Entry<String, String> entry : localFallbackCache.entrySet()) {
            if (!resultMap.containsKey(entry.getKey())) {
                MessageQueueDefinition def = fromJson(entry.getValue());
                if (def != null && def.getQueueId() != null) {
                    resultMap.put(def.getQueueId(), def);
                }
            }
        ...
        if (resultMap.isEmpty()) {
            ...
            List<MessageQueueDefinition> seeded = seedDefaultQueues(queueConfig);
            for (MessageQueueDefinition q : seeded) {
    ...
    public synchronized List<MessageQueueDefinition> seedDefaultQueues(QueueConfig config) {
        List<MessageQueueDefinition> seededList = new ArrayList<>();

        String taskQueue = (config != null) ? config.getQueueName() : QueueConfig.DEFAULT_QUEUE_NAME;
        MessageQueueDefinition processingQueue = new MessageQueueDefinition(
        ...
        MessageQueueDefinition baseEventQueue = new MessageQueueDefinition(
        ...
        MessageQueueDefinition defaultGwQueue = new MessageQueueDefinition(
        ...
        if (config != null) {
            for (String qName : config.getGatewayEventQueues()) {
                if (!qName.equals(taskQueue) && !qName.equals(eventQueue) && !qName.equals(defaultGateway)) {
                    MessageQueueDefinition extraQueue = new MessageQueueDefinition(
