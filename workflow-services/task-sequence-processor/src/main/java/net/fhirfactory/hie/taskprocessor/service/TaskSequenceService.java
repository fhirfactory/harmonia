...
import net.fhirfactory.hie.taskprocessor.sequence.TaskSequenceImplementation;
...
/**
 * Service managing persisted {@link TaskSequenceDefinition} configuration objects in the Infinispan cluster.
 * <p>
...
public class TaskSequenceService {
    ...
    public TaskSequenceService() {
    }

    public TaskSequenceService(RemoteCacheManager remoteCacheManager) {
        this.remoteCacheManager = remoteCacheManager;
        this.objectMapper = new ObjectMapper();
    }

    public TaskSequenceService(RemoteCacheManager remoteCacheManager, ObjectMapper objectMapper) {
        this.remoteCacheManager = remoteCacheManager;
    ...
    public String toJson(TaskSequenceDefinition sequence) {
        if (sequence == null) {
            return null;
        }
        try {
            ...
            log.error("Failed to serialize TaskSequenceDefinition [{}] to JSON: {}", sequence.getSequenceId(), e.getMessage(), e);
            throw new RuntimeException("Serialization error: " + e.getMessage(), e);
        }
    }

    public TaskSequenceDefinition fromJson(String json) {
        if (StringUtils.isBlank(json)) {
            return null;
        }
        try {
            return getObjectMapper().readValue(json, TaskSequenceDefinition.class);
        } catch (Exception e) {
    ...
    public <T extends TaskSequenceDefinition> T fromJson(String json, Class<T> clazz) {
        if (StringUtils.isBlank(json)) {
            return null;
        }
        try {
    ...
    public TaskSequenceDefinition save(TaskSequenceDefinition sequence) {
        if (sequence == null) {
        ...
        if (sequence instanceof TaskSequenceImplementation) {
            ((TaskSequenceImplementation) sequence).syncActivityIds();
        }
    ...
    public TaskSequenceDefinition create(TaskSequenceDefinition sequence) {
        return save(sequence);
    }

    public TaskSequenceDefinition update(TaskSequenceDefinition sequence) {
        return save(sequence);
    }

    public Optional<TaskSequenceDefinition> getById(String sequenceId) {
        if (StringUtils.isBlank(sequenceId)) {
    ...
    public List<TaskSequenceDefinition> getAll() {
        Map<String, TaskSequenceDefinition> sequenceMap = new LinkedHashMap<>();
        RemoteCache<String, String> remoteCache = getRemoteCache();
        if (remoteCache != null) {
            try {
                ...
                if (values != null && !values.isEmpty()) {
                    for (String json : values) {
                        if (json != null) {
                            try {
                                TaskSequenceDefinition seq = fromJson(json);
                                if (seq != null) {
                                    sequenceMap.put(seq.getSequenceId(), seq);
                                }
        ...
        for (Map.Entry<String, String> entry : localFallbackCache.entrySet()) {
            if (!sequenceMap.containsKey(entry.getKey())) {
                try {
                    TaskSequenceDefinition seq = fromJson(entry.getValue());
                    if (seq != null) {
                        sequenceMap.put(seq.getSequenceId(), seq);
                    }
                } catch (Exception ignored) {}
    ...
    public List<TaskSequenceDefinition> findMatching(String gatewayInstanceId, String triggerType) {
        return getAll().stream()
                .filter(TaskSequenceDefinition::isEnabled)
                .filter(seq -> seq.matches(gatewayInstanceId, triggerType))
                .collect(Collectors.toList());
    }

    public List<TaskSequenceDefinition> findByGateway(String gatewayInstanceId) {
        return getAll().stream()
                .filter(TaskSequenceDefinition::isEnabled)
    ...
    public List<TaskSequenceDefinition> findByTrigger(String triggerType) {
        return getAll().stream()
                .filter(TaskSequenceDefinition::isEnabled)
