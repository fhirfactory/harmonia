...
package net.fhirfactory.hie.model.queue;
...
class MessageQueueDefinitionTest {

    private final ObjectMapper mapper = new ObjectMapper();

    @Test
    void testConstructorsAndProperties() {
        MessageQueueDefinition q = new MessageQueueDefinition("task.processing.queue", "ANYCAST", false, "Task processing queue");
        ...
        MessageQueueDefinition q2 = new MessageQueueDefinition("gw-queue", "task.event.queue.pas-gw", "task.event.queue.pas-gw", "ANYCAST", true, "PAS queue", "pas-gw");
    ...
    void testSerializationAndDeserialization() throws Exception {
        MessageQueueDefinition original = new MessageQueueDefinition("q1", "task.test.queue", "task.test.queue", "ANYCAST", false, "Test Queue", "test-gw");
        original.setMaxConsumers(10);
        original.setFilter("HIE_TRIGGER_TYPE = 'A01'");

        String json = mapper.writeValueAsString(original);
        MessageQueueDefinition deserialized = mapper.readValue(json, MessageQueueDefinition.class);
